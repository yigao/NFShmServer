...
bool NFCProxyClientModule::Awake()
{
    ...
    NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER_FOR_CLIENT, proto_game::NF_CS_MSG_EnterGame_AUTO_JOIN_Req,
                                                 this,
                                                 &NFCProxyClientModule::OnHandleEnterGameAutoJoinFromClient);
    NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER_FOR_CLIENT, proto_game::NF_CS_MSG_EnterGameReq,
                                                 this,
                                                 &NFCProxyClientModule::OnHandleEnterGameReqFromClient);
    NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER_FOR_CLIENT, proto_game::NF_CS_MSG_ExitRoomReq,
                                                 this,
                                                 &NFCProxyClientModule::OnHandleExitRoomReqFromClient);
    NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER_FOR_CLIENT, proto_game::NF_CS_MSG_ExitGameReq,
                                                 this,
                                                 &NFCProxyClientModule::OnHandleWorldMsgFromClient);

    NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER_FOR_CLIENT, proto_game::NF_CS_MSG_DeskListReq,
                                                 this,
                                                 &NFCProxyClientModule::OnHandleWorldMsgFromClient);
    NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER_FOR_CLIENT, proto_game::NF_CS_MSG_ChairCheckReq,
                                                 this,
                                                 &NFCProxyClientModule::OnHandleWorldMsgFromClient);
    /////来自Login Server的协议////////////////////////////////////////
    ...
    NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER, proto_game::NF_CS_MSG_EnterGame_AUTO_JOIN_Rsp,
                                                       this,
                                                       &NFCProxyClientModule::OnHandlePlayerEnterGameAutoJoinRsp);

	NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER, proto_game::NF_SC_MSG_ExitGameRsp,
                                                       this,
                                                       &NFCProxyClientModule::OnHandleExitGameReqFromClientRsp);

	NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER, proto_game::NF_SC_MSG_DeskListRsp,
                                                       this,
                                                       &NFCProxyClientModule::OnHandleClientRspFromWorldServer);
	NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER, proto_game::NF_SC_MSG_ChairCheckRsp,
                                                       this,
                                                       &NFCProxyClientModule::OnHandleClientRspFromWorldServer);
	NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER, proto_game::NF_SC_MSG_EnterGameRsp,
														this,
														&NFCProxyClientModule::OnHandleEnterGameReqFromClientRsp);
	NFMessageMgr::Instance()->AddMessageCallBack(NF_ST_PROXY_SERVER, proto_game::NF_SC_MSG_ExitRoomRsp,
                                                       this,
                                                       &NFCProxyClientModule::OnHandleExitRoomReqFromClientRsp);
...
int NFCProxyClientModule::OnHandleProxyClientOtherMessage(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId,
    ...
    if (nMsgId == proto_game::NF_CS_MSG_EnterGame_AUTO_JOIN_Req ||
        nMsgId == proto_game::NF_CS_MSG_DeskListReq ||
        nMsgId == proto_game::NF_CS_MSG_ChairCheckReq ||
        nMsgId == proto_game::NF_CS_MSG_EnterGameReq ||
        nMsgId == proto_game::NF_CS_MSG_ExitRoomReq ||
        nMsgId == proto_game::NF_CS_MSG_ExitGameReq ||
        nMsgId == proto_game::NF_CS_Msg_Get_Room_Info_Req)
    {
        NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "recv nMsgId = {}, transfer to world server", nMsgId);
        if (pPlayerInfo->mWorldBusId > 0) {
            NFMessageMgr::Instance()->SendProxyMsgByBusId(NF_ST_PROXY_SERVER, pPlayerInfo->mWorldBusId, nMsgId, msg, nLen,
    ...
    else
    {
        ...
        else
        {
            ...
            if (pPlayerInfo->mGameBusId > 0) {
                NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, pPlayerInfo->mPlayerId, "recv mainMsgId:{} subMsgId:{}, transfer to game server", nMainMsgId, nSubMsgId);
                NFMessageMgr::Instance()->SendProxyMsgByBusId(NF_ST_PROXY_SERVER, pPlayerInfo->mGameBusId, nMsgId, msg, nLen,
                                                         pPlayerInfo->mPlayerId);
            }
            else
            {
                NFLogError(NF_LOG_PROXY_CLIENT_PLUGIN, pPlayerInfo->mPlayerId, "recv mainMsgId:{} subMsgId:{}, not transfer to game server", nMainMsgId, nSubMsgId);
            }
        }
    }
...
int NFCProxyClientModule::OnHandleExitGameReqFromClientRsp(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");
    ...
    if (pPlayerInfo == nullptr || !pPlayerInfo->mIsLogin)
    {
        NFLogWarning(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "playerId:{} not login, recv msg:{}", playerId, nMsgId);
        return 0;
    }
...
int NFCProxyClientModule::OnHandlePlayerEnterGameAutoJoinRsp(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");
    proto_game::EnterGameAutoJoinRsp xMsg;
    CLIENT_MSG_PROCESS_WITH_PRINTF(nMsgId, playerId, msg, nLen, xMsg);

    NF_SHARE_PTR<NFProxyPlayerInfo> pPlayerLinkInfo = mPlayerLinkInfo.GetElement(xMsg.player_id());
    CHECK_PLAYER_EXPR(xMsg.player_id(), pPlayerLinkInfo, -1,
                      "Player:{} error, can't find playerinfo in the proxy server",
                      xMsg.player_id());
    ...
    NFMessageMgr::Instance()->Send(pPlayerLinkInfo->mLinkId, nMsgId, xMsg, xMsg.player_id());
    NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- end --");
    return 0;
}

int NFCProxyClientModule::OnHandleEnterGameReqFromClientRsp(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
	NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");
    proto_game::EnterGameRsp xMsg;
	CLIENT_MSG_PROCESS_WITH_PRINTF(nMsgId, playerId, msg, nLen, xMsg);

	NF_SHARE_PTR<NFProxyPlayerInfo> pPlayerLinkInfo = mPlayerLinkInfo.GetElement(playerId);
	CHECK_PLAYER_EXPR(playerId, pPlayerLinkInfo, -1,
		"Player:{} error, can't find playerinfo in the proxy server",
		playerId);
	...
	NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "pPlayerLinkInfo->mGameId = {}",    pPlayerLinkInfo->mGameId);
	NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "pPlayerLinkInfo->mRoomId = {}",    pPlayerLinkInfo->mRoomId);
	NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "pPlayerLinkInfo->mGameBusId = {}", pPlayerLinkInfo->mGameBusId);


	NFMessageMgr::Instance()->Send(pPlayerLinkInfo->mLinkId, nMsgId, xMsg, playerId);
	NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- end --");
	return 0;
}

int NFCProxyClientModule::OnHandleExitRoomReqFromClientRsp(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
	NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");
    proto_game::ExitRoomRsp xMsg;
	CLIENT_MSG_PROCESS_WITH_PRINTF(nMsgId, playerId, msg, nLen, xMsg);
	...
	if (xMsg.result() == 0)
	{
		pPlayerLinkInfo->mGameId = 0;
		pPlayerLinkInfo->mRoomId = 0;
		pPlayerLinkInfo->mGameBusId = 0;
	}
...
int NFCProxyClientModule::OnHandleEnterGameAutoJoinFromClient(uint64_t unLinkId, uint64_t value, uint64_t value2, uint32_t nMsgId,
                                                              const char *msg, uint32_t nLen) 
{
    NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");
    proto_game::EnterGameAutoJoinReq xMsg;
    CLIENT_MSG_PROCESS_WITH_PRINTF(nMsgId, value, msg, nLen, xMsg);

    std::string ip = NFMessageMgr::Instance()->GetLinkIp(unLinkId);
    NF_SHARE_PTR<NFProxySession> pLinkInfo = mClientLinkInfo.GetElement(unLinkId);
    if (pLinkInfo == nullptr) {
        NFLogWarning(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "ip:{} can't find the linkinfo ,recv msg:{}",
                     ip, nMsgId);
        return 0;
    }
    ...
    NF_SHARE_PTR<NFProxyPlayerInfo> pPlayerInfo = mPlayerLinkInfo.GetElement(pLinkInfo->mPlayerId);
    if (pPlayerInfo == nullptr || !pPlayerInfo->mIsLogin) {
        NFLogWarning(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "ip:{} not login,recv msg:{}",
                     ip, nMsgId);
    ...
    if (pPlayerInfo->mGameId > 0 || pPlayerInfo->mRoomId > 0) {
        if (xMsg.game_id() != pPlayerInfo->mGameId || xMsg.room_id() != pPlayerInfo->mRoomId) {
            NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, pPlayerInfo->mPlayerId,
                       "player will enter a diff game room, not leave old room, gameId:{} roomId:{} oldGameId:{}, oldRoomId:{}",
                       xMsg.game_id(), xMsg.room_id(),
                       pPlayerInfo->mGameId, pPlayerInfo->mRoomId);
        }
    }
...
int NFCProxyClientModule::OnHandleEnterGameReqFromClient(uint64_t unLinkId, uint64_t value, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
	NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");
	...
	NFMessageMgr::Instance()->SendProxyMsgByBusId(NF_ST_PROXY_SERVER, pPlayerInfo->mWorldBusId,
													nMsgId, xMsg, pPlayerInfo->mPlayerId);
...
int NFCProxyClientModule::OnHandleExitRoomReqFromClient(uint64_t unLinkId, uint64_t value, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
	NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");
...
int NFCProxyClientModule::OnHandleGameOtherMessage(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
	NF_SHARE_PTR<NFProxyPlayerInfo> pPlayerInfo = mPlayerLinkInfo.GetElement(playerId);
	if (pPlayerInfo)
	{
        uint16_t nMainMsgId = HIGH_UINT16(nMsgId);
        uint16_t nSubMsgId = LOW_UINT16(nMsgId);
	    NFLogTrace(NF_LOG_SYSTEMLOG, playerId, "proxy trans game server mainMsgId:{} subMsgId:{} to client", nMainMsgId, nSubMsgId);
		NFMessageMgr::Instance()->Send(pPlayerInfo->mLinkId, nMsgId, msg, nLen, 0);
	}
	else
	{
		NFLogError(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "game server msg:{} not handle", nMsgId);
	}

	return 0;
}

int NFCProxyClientModule::OnHandleWorldOtherMessage(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
	NF_SHARE_PTR<NFProxyPlayerInfo> pPlayerInfo = mPlayerLinkInfo.GetElement(playerId);
	if (pPlayerInfo)
	{
        NFLogTrace(NF_LOG_SYSTEMLOG, playerId, "proxy trans world server msg:{} to client", nMsgId);
		NFMessageMgr::Instance()->Send(pPlayerInfo->mLinkId, nMsgId, msg, nLen, 0);
	}
	else
	{
		NFLogError(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "world server msg:{} not handle", nMsgId);
	}
	return 0;
}

int NFCProxyClientModule::OnHandleLoginOtherMessage(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
	NF_SHARE_PTR<NFProxyPlayerInfo> pPlayerInfo = mPlayerLinkInfo.GetElement(playerId);
	if (pPlayerInfo)
	{
		NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, playerId, "tranfer login server msgId:{} to client", nMsgId);
		NFMessageMgr::Instance()->Send(pPlayerInfo->mLinkId, nMsgId, msg, nLen, 0);
	}
	else
	{
		NFLogError(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "login server msg:{} not handle", nMsgId);
	}

	return 0;
}

int NFCProxyClientModule::OnHandleLogicOtherMessage(uint64_t unLinkId, uint64_t playerId, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");
    ...
    if (pPlayerInfo)
    {
        NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, playerId, "tranfer logic server msgId:{} to client", nMsgId);
        NFMessageMgr::Instance()->Send(pPlayerInfo->mLinkId, nMsgId, msg, nLen, 0);
    }
    else
    {
        NFLogError(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "logic server msg:{} not handle", nMsgId);
    }
...
int NFCProxyClientModule::OnHandleWorldMsgFromClient(uint64_t unLinkId, uint64_t value, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "-- begin --");

    NFLogTrace(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "unLinkId = {} , msgId = {}", unLinkId, nMsgId);

    std::string ip = NFMessageMgr::Instance()->GetLinkIp(unLinkId);
    NF_SHARE_PTR<NFProxySession> pLinkInfo = mClientLinkInfo.GetElement(unLinkId);
    if (pLinkInfo == nullptr) {
        NFLogWarning(NF_LOG_PROXY_CLIENT_PLUGIN, 0, "ip:{} can't find the linkinfo ,recv msg:{}", ip, nMsgId);
