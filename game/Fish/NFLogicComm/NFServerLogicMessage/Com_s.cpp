...
pbFishPlayerSimpleData_s::pbFishPlayerSimpleData_s() {
	if (EN_OBJ_MODE_INIT == NFShmMgr::Instance()->GetCreateMode()) {
		CreateInit();
	} else {
		ResumeInit();
...
int pbFishPlayerSimpleData_s::CreateInit() {
...
int pbFishPlayerSimpleData_s::ResumeInit() {
	return 0;
}

void pbFishPlayerSimpleData_s::write_to_pbmsg(::proto_ff::pbFishPlayerSimpleData & msg) const {
...
void pbFishPlayerSimpleData_s::read_from_pbmsg(const ::proto_ff::pbFishPlayerSimpleData & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct pbFishPlayerSimpleData_s));
