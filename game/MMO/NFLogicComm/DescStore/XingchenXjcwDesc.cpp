...
int XingchenXjcwDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_xingchenxjcw_list_size() < 0) || (table.e_xingchenxjcw_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_xingchenxjcw_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_xjslot();
		}
		else
		{
			if (desc.has_m_xjslot() < m_minId)
			{
				m_minId = desc.has_m_xjslot();
			}
		}
		...
		auto pDesc = &m_astDescMap[desc.m_xjslot()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_xjslot());
		pDesc->read_from_pbmsg(desc);
