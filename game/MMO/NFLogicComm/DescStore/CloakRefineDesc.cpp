#include "CloakRefineDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

CloakRefineDesc::CloakRefineDesc()
{
...
int CloakRefineDesc::CreateInit()
{
	return 0;
}

int CloakRefineDesc::ResumeInit()
{
	return 0;
}

int CloakRefineDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "CloakRefineDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_CloakRefine table;
	...
	if ((table.e_cloakrefine_list_size() < 0) || (table.e_cloakrefine_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_cloakrefine_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_cloakrefine_list_size(); i++)
	{
		const proto_ff::E_CloakRefine& desc = table.e_cloakrefine_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_cloakrefine_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int CloakRefineDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
