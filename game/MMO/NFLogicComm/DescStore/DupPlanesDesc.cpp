...
DupPlanesDesc::~DupPlanesDesc()
{
}

int DupPlanesDesc::CreateInit()
{
	return Initialize();
}

int DupPlanesDesc::ResumeInit()
{
	return 0;
}

int DupPlanesDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "DupPlanesDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_DupPlanes table;
	...
	if ((table.e_dupplanes_list_size() < 0) || (table.e_dupplanes_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_dupplanes_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_dupplanes_list_size(); i++)
	{
		const proto_ff::E_DupPlanes& desc = table.e_dupplanes_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_dupplanes_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int DupPlanesDesc::CheckWhenAllDataLoaded()
{
	return 0;
}

const proto_ff_s::E_DupPlanes_s * DupPlanesDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_DupPlanes_s * DupPlanesDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_DupPlanes_s *>((static_cast<const DupPlanesDesc*>(this))->GetDesc(id));
}

int DupPlanesDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_DupPlanes_s * DupPlanesDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_DupPlanes_s * DupPlanesDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
