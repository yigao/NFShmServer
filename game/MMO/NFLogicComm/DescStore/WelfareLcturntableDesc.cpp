...
WelfareLcturntableDesc::~WelfareLcturntableDesc()
{
}

int WelfareLcturntableDesc::CreateInit()
{
	return Initialize();
}

int WelfareLcturntableDesc::ResumeInit()
{
	return 0;
}

int WelfareLcturntableDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "WelfareLcturntableDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_WelfareLcturntable table;
	...
	if ((table.e_welfarelcturntable_list_size() < 0) || (table.e_welfarelcturntable_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_welfarelcturntable_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_welfarelcturntable_list_size(); i++)
	{
		const proto_ff::E_WelfareLcturntable& desc = table.e_welfarelcturntable_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_welfarelcturntable_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int WelfareLcturntableDesc::CheckWhenAllDataLoaded()
{
	return 0;
}

const proto_ff_s::E_WelfareLcturntable_s * WelfareLcturntableDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_WelfareLcturntable_s * WelfareLcturntableDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_WelfareLcturntable_s *>((static_cast<const WelfareLcturntableDesc*>(this))->GetDesc(id));
}

int WelfareLcturntableDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_WelfareLcturntable_s * WelfareLcturntableDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_WelfareLcturntable_s * WelfareLcturntableDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
