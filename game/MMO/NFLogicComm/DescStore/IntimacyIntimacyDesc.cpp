...
int IntimacyIntimacyDesc::Load(NFResDB *pDB)
{
	...
	if ((table.intimacyintimacy_list_size() < 0) || (table.intimacyintimacy_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.intimacyintimacy_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.intimacyintimacy_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.intimacyintimacy_list_size(); i++)
	{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.intimacyintimacy_list_size());
...
{
	return const_cast<proto_ff_s::intimacyintimacy_s *>((static_cast<const IntimacyIntimacyDesc*>(this))->GetDesc(id));
}
