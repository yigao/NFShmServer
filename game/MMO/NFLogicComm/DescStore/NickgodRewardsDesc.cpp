#include "NickgodRewardsDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

NickgodRewardsDesc::NickgodRewardsDesc()
{
...
int NickgodRewardsDesc::CreateInit()
{
	return 0;
}

int NickgodRewardsDesc::ResumeInit()
{
	return 0;
}

int NickgodRewardsDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "NickgodRewardsDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_NickgodRewards table;
	...
	if ((table.e_nickgodrewards_list_size() < 0) || (table.e_nickgodrewards_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_nickgodrewards_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_nickgodrewards_list_size(); i++)
	{
		const proto_ff::E_NickgodRewards& desc = table.e_nickgodrewards_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_nickgodrewards_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int NickgodRewardsDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
