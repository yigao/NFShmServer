#include "Soul3dmodelDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

Soul3dmodelDesc::Soul3dmodelDesc()
{
...
int Soul3dmodelDesc::CreateInit()
{
	return 0;
}

int Soul3dmodelDesc::ResumeInit()
{
	return 0;
}

int Soul3dmodelDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "Soul3dmodelDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_Soul3dmodel table;
	...
	if ((table.e_soul3dmodel_list_size() < 0) || (table.e_soul3dmodel_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_soul3dmodel_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_soul3dmodel_list_size(); i++)
	{
		const proto_ff::E_Soul3dmodel& desc = table.e_soul3dmodel_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_soul3dmodel_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int Soul3dmodelDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
