...
int MonsterDropDesc::Load(NFResDB *pDB)
{
	...
	if ((table.monsterdrop_list_size() < 0) || (table.monsterdrop_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.monsterdrop_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.monsterdrop_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.monsterdrop_list_size(); i++)
	{
		const proto_ff::monsterdrop& desc = table.monsterdrop_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.monsterdrop_list_size());
...
const proto_ff_s::monsterdrop_s * MonsterDropDesc::GetDesc(int id) const
{
...
proto_ff_s::monsterdrop_s * MonsterDropDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::monsterdrop_s *>((static_cast<const MonsterDropDesc*>(this))->GetDesc(id));
}
