...
int SkillSkillDesc::Load(NFResDB *pDB)
{
	...
	if ((table.skillskill_list_size() < 0) || (table.skillskill_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.skillskill_list_size());
		return -2;
	}

	m_astDesc.resize(table.skillskill_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.skillskill_list_size(); i++)
	{
		const proto_ff::skillskill& desc = table.skillskill_list(i);
		...
		auto iter = m_astDescMap.emplace_hint(desc.skillid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.skillid());
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.skillskill_list_size());
...
proto_ff_s::skillskill_s * SkillSkillDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::skillskill_s *>((static_cast<const SkillSkillDesc*>(this))->GetDesc(id));
}
