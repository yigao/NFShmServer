...
XingchenXjattDesc::~XingchenXjattDesc()
{
}

int XingchenXjattDesc::CreateInit()
{
	return Initialize();
}

int XingchenXjattDesc::ResumeInit()
{
	return 0;
}

int XingchenXjattDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "XingchenXjattDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_XingchenXjatt table;
	...
	if ((table.e_xingchenxjatt_list_size() < 0) || (table.e_xingchenxjatt_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_xingchenxjatt_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_xingchenxjatt_list_size(); i++)
	{
		const proto_ff::E_XingchenXjatt& desc = table.e_xingchenxjatt_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_xingchenxjatt_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int XingchenXjattDesc::CheckWhenAllDataLoaded()
{
	return 0;
}

const proto_ff_s::E_XingchenXjatt_s * XingchenXjattDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_XingchenXjatt_s * XingchenXjattDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_XingchenXjatt_s *>((static_cast<const XingchenXjattDesc*>(this))->GetDesc(id));
}

int XingchenXjattDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_XingchenXjatt_s * XingchenXjattDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_XingchenXjatt_s * XingchenXjattDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
