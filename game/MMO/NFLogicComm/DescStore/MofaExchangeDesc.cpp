#include "MofaExchangeDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

IMPLEMENT_IDCREATE_WITHTYPE(MofaExchangeDesc, EOT_CONST_MOFA_EXCHANGE_DESC_ID, NFShmObj)

MofaExchangeDesc::MofaExchangeDesc():NFIDescStore()
{
...
MofaExchangeDesc::~MofaExchangeDesc()
{
}

int MofaExchangeDesc::CreateInit()
{
	return Initialize();
}

int MofaExchangeDesc::ResumeInit()
{
	return 0;
}

int MofaExchangeDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "MofaExchangeDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_MofaExchange table;
	...
	if ((table.e_mofaexchange_list_size() < 0) || (table.e_mofaexchange_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_mofaexchange_list_size());
		return -2;
	}
...
int MofaExchangeDesc::CheckWhenAllDataLoaded()
{
	return 0;
}

const proto_ff_s::E_MofaExchange_s * MofaExchangeDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_MofaExchange_s * MofaExchangeDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_MofaExchange_s *>((static_cast<const MofaExchangeDesc*>(this))->GetDesc(id));
}

int MofaExchangeDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_MofaExchange_s * MofaExchangeDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_MofaExchange_s * MofaExchangeDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
