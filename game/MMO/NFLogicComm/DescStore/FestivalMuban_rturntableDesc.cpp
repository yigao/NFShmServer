...
int FestivalMuban_rturntableDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalMuban_rturntableDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_FestivalMuban_rturntable table;
	...
	if ((table.e_festivalmuban_rturntable_list_size() < 0) || (table.e_festivalmuban_rturntable_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivalmuban_rturntable_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_festivalmuban_rturntable_list_size(); i++)
	{
		const proto_ff::E_FestivalMuban_rturntable& desc = table.e_festivalmuban_rturntable_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_festivalmuban_rturntable_list_size());
