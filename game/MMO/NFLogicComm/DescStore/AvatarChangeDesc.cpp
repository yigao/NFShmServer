...
int AvatarChangeDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "AvatarChangeDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_AvatarChange table;
	...
	if ((table.e_avatarchange_list_size() < 0) || (table.e_avatarchange_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_avatarchange_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_avatarchange_list_size(); i++)
	{
		const proto_ff::E_AvatarChange& desc = table.e_avatarchange_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_avatarchange_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int AvatarChangeDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		auto pDesc = &m_astDesc[i];
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_starid), result, "can't find the starid:{} in the  excel:item sheet:item", pDesc->m_starid);
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_activationitem), result, "can't find the activationitem:{} in the  excel:item sheet:item", pDesc->m_activationitem);
	}
	return result;
}
