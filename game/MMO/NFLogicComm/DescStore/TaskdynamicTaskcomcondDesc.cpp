...
IMPLEMENT_IDCREATE_WITHTYPE_GLOBAL(TaskdynamicTaskcomcondDesc, EOT_CONST_TASKDYNAMIC_TASKCOMCOND_DESC_ID, NFShmObj)
...
int TaskdynamicTaskcomcondDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_taskdynamictaskcomcond_list_size() < 0) || (table.e_taskdynamictaskcomcond_list_size() > (int)(m_astDesc.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_taskdynamictaskcomcond_list_size(); i++)
	{
		...
		if (desc.has_m_comcondid() == false && desc.ByteSize() == 0)
		{
		...
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_comcondid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_comcondid(), curIndex);
		CHECK_EXPR_ASSERT(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_comcondid());
...
const proto_ff_s::E_TaskdynamicTaskcomcond_s * TaskdynamicTaskcomcondDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_TaskdynamicTaskcomcond_s * TaskdynamicTaskcomcondDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_TaskdynamicTaskcomcond_s *>((static_cast<const TaskdynamicTaskcomcondDesc*>(this))->GetDesc(id));
}

int TaskdynamicTaskcomcondDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_TaskdynamicTaskcomcond_s * TaskdynamicTaskcomcondDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_TaskdynamicTaskcomcond_s * TaskdynamicTaskcomcondDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
