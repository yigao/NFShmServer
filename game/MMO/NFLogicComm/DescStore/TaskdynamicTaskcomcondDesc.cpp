...
int TaskdynamicTaskcomcondDesc::Load(NFResDB *pDB)
{
	...
	if ((table.taskdynamictaskcomcond_list_size() < 0) || (table.taskdynamictaskcomcond_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.taskdynamictaskcomcond_list_size());
		return -2;
	}

	m_astDesc.resize(table.taskdynamictaskcomcond_list_size());
	...
	for (int i = 0; i < (int)table.taskdynamictaskcomcond_list_size(); i++)
	{
		...
		auto iter = m_astDescMap.emplace_hint(desc.comcondid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.comcondid());
