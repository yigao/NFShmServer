...
int QypeakBackDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_qypeakback_list_size() < 0) || (table.e_qypeakback_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_qypeakback_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_layers();
		}
		else
		{
			if (desc.has_m_layers() < m_minId)
			{
				m_minId = desc.has_m_layers();
			}
		}
		...
		auto pDesc = &m_astDescMap[desc.m_layers()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_layers());
		pDesc->read_from_pbmsg(desc);
