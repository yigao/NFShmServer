...
int DailyHelperDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_dailyhelper_list_size() < 0) || (table.e_dailyhelper_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_dailyhelper_list_size(); i++)
	{
		...
		auto pDesc = &m_astDescMap[desc.m_helperid()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_helperid());
		pDesc->read_from_pbmsg(desc);
