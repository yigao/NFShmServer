...
int DailySportsDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "DailySportsDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_DailySports table;
	...
	if ((table.e_dailysports_list_size() < 0) || (table.e_dailysports_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_dailysports_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_dailysports_list_size(); i++)
	{
		const proto_ff::E_DailySports& desc = table.e_dailysports_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_dailysports_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int DailySportsDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		auto pDesc = &m_astDesc[i];
		CHECK_EXPR_MSG_RESULT(PeractivityPeractivityDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_peractivityid), result, "can't find the peractivityid:{} in the  excel:perActivity sheet:perActivity", pDesc->m_peractivityid);
		CHECK_EXPR_MSG_RESULT(FunctionunlockFunctionunlockDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_functionid), result, "can't find the functionid:{} in the  excel:functionunlock sheet:functionUnlock", pDesc->m_functionid);
	}
	return result;
}
