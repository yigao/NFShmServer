...
int FestivalMuban_play_scoreDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalMuban_play_scoreDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_FestivalMuban_play_score table;
	...
	if ((table.e_festivalmuban_play_score_list_size() < 0) || (table.e_festivalmuban_play_score_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivalmuban_play_score_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_festivalmuban_play_score_list_size(); i++)
	{
		const proto_ff::E_FestivalMuban_play_score& desc = table.e_festivalmuban_play_score_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_festivalmuban_play_score_list_size());
