...
DailyCultivateDesc::~DailyCultivateDesc()
{
}
...
int DailyCultivateDesc::ResumeInit()
{
	return 0;
}

int DailyCultivateDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "DailyCultivateDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_DailyCultivate table;
	...
	if ((table.e_dailycultivate_list_size() < 0) || (table.e_dailycultivate_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_dailycultivate_list_size());
		return -2;
	}
...
int DailyCultivateDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		{
			CHECK_EXPR_MSG_RESULT(AttributeAttributeDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_attribute[j].m_id), result, "can't find the attribute:{} in the  excel:attribute sheet:attribute", pDesc->m_attribute[j].m_id);
		}
	}
	return result;
}

const proto_ff_s::E_DailyCultivate_s * DailyCultivateDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_DailyCultivate_s * DailyCultivateDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_DailyCultivate_s *>((static_cast<const DailyCultivateDesc*>(this))->GetDesc(id));
}

int DailyCultivateDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_DailyCultivate_s * DailyCultivateDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_DailyCultivate_s * DailyCultivateDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
