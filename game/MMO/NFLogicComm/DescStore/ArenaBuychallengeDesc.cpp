...
int ArenaBuychallengeDesc::Load(NFResDB *pDB)
{
	...
	if ((table.arenabuychallenge_list_size() < 0) || (table.arenabuychallenge_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.arenabuychallenge_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.arenabuychallenge_list_size());
	...
	for (int i = 0; i < (int)table.arenabuychallenge_list_size(); i++)
	{
		...
		auto pIndex = m_astDescMap.Insert(desc.count());
		CHECK_EXPR(pIndex, -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.count());
		*pIndex = i;
