...
int ArenaBuychallengeDesc::Load(NFResDB *pDB)
{
	...
	if ((table.arenabuychallenge_list_size() < 0) || (table.arenabuychallenge_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.arenabuychallenge_list_size());
		return -2;
	}

	m_astDesc.resize(table.arenabuychallenge_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.arenabuychallenge_list_size(); i++)
	{
		const proto_ff::arenabuychallenge& desc = table.arenabuychallenge_list(i);
		if (desc.has_count() == false && desc.ByteSize() == 0)
		{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.count());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.count(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.count());
		uint64_t hashKey = desc.count();
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.arenabuychallenge_list_size());
...
{
	return const_cast<proto_ff_s::arenabuychallenge_s *>((static_cast<const ArenaBuychallengeDesc*>(this))->GetDesc(id));
}
