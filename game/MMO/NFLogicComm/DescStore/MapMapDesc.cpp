...
int MapMapDesc::Load(NFResDB *pDB)
{
	...
	if ((table.mapmap_list_size() < 0) || (table.mapmap_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.mapmap_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.mapmap_list_size());
	...
	for (int i = 0; i < (int)table.mapmap_list_size(); i++)
	{
		...
		auto pIndex = m_astDescMap.Insert(desc.mapid());
		CHECK_EXPR(pIndex, -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.mapid());
		*pIndex = i;
