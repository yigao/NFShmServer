#pragma once

#include "NFServerComm/NFDescStorePlugin/NFIDescStore.h"
#include "NFComm/NFShmCore/NFShmMgr.h"
#include "NFLogicCommon/NFDescStoreTypeDefines.h"
#include "NFServerLogicMessage/activity_s.h"

#define MAX_ACTIVITY_MATHRULE_NUM 40

class ActivityMathruleDesc : public NFIDescStore
{
public:
	ActivityMathruleDesc(NFIPluginManager* pPluginManager);
	virtual ~ActivityMathruleDesc();
	const proto_ff_s::activitymathrule_s* GetDesc(int id) const;
	proto_ff_s::activitymathrule_s* GetDesc(int id);
	const NFShmHashMap<uint64_t, proto_ff_s::activitymathrule_s, MAX_ACTIVITY_MATHRULE_NUM>* GetAllDesc() const { return &m_astDesc; }
	NFShmHashMap<uint64_t, proto_ff_s::activitymathrule_s, MAX_ACTIVITY_MATHRULE_NUM>* GetAllDesc() { return &m_astDesc; }
public:
IMPL_RES_HASH_DESC(proto_ff_s::activitymathrule_s, activitymathrule, MAX_ACTIVITY_MATHRULE_NUM);
DECLARE_IDCREATE(ActivityMathruleDesc);
};
