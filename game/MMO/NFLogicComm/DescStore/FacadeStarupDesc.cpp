...
IMPLEMENT_IDCREATE_WITHTYPE(FacadeStarupDesc, EOT_CONST_FACADE_STARUP_DESC_ID, NFShmObj)

FacadeStarupDesc::FacadeStarupDesc():NFIDescStore()
{
...
int FacadeStarupDesc::Load(NFResDB *pDB)
{
	...
	if ((table.facadestarup_list_size() < 0) || (table.facadestarup_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.facadestarup_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.facadestarup_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.facadestarup_list_size(); i++)
	{
		const proto_ff::facadestarUp& desc = table.facadestarup_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.facadestarup_list_size());
...
const proto_ff_s::facadestarUp_s * FacadeStarupDesc::GetDesc(int id) const
{
...
proto_ff_s::facadestarUp_s * FacadeStarupDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::facadestarUp_s *>((static_cast<const FacadeStarupDesc*>(this))->GetDesc(id));
}
