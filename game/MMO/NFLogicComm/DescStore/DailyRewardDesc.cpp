...
DailyRewardDesc::~DailyRewardDesc()
{
}
...
int DailyRewardDesc::ResumeInit()
{
	return 0;
}

int DailyRewardDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "DailyRewardDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_DailyReward table;
	...
	if ((table.e_dailyreward_list_size() < 0) || (table.e_dailyreward_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_dailyreward_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_dailyreward_list_size(); i++)
	{
		const proto_ff::E_DailyReward& desc = table.e_dailyreward_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_dailyreward_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int DailyRewardDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		auto pDesc = &m_astDesc[i];
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_rewardid), result, "can't find the rewardid:{} in the  excel:item sheet:item", pDesc->m_rewardid);
	}
	return result;
}

const proto_ff_s::E_DailyReward_s * DailyRewardDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_DailyReward_s * DailyRewardDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_DailyReward_s *>((static_cast<const DailyRewardDesc*>(this))->GetDesc(id));
}

int DailyRewardDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_DailyReward_s * DailyRewardDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_DailyReward_s * DailyRewardDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
