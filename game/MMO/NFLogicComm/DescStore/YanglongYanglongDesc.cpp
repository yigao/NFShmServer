...
IMPLEMENT_IDCREATE_WITHTYPE(YanglongYanglongDesc, EOT_CONST_YANGLONG_YANGLONG_DESC_ID, NFShmObj)

YanglongYanglongDesc::YanglongYanglongDesc():NFIDescStore()
{
...
int YanglongYanglongDesc::Load(NFResDB *pDB)
{
	...
	for (int i = 0; i < (int)table.e_yanglongyanglong_list_size(); i++)
	{
		...
		uint64_t hashKey = desc.m_projectid();
...
const proto_ff_s::E_YanglongYanglong_s * YanglongYanglongDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_YanglongYanglong_s * YanglongYanglongDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_YanglongYanglong_s *>((static_cast<const YanglongYanglongDesc*>(this))->GetDesc(id));
}

int YanglongYanglongDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_YanglongYanglong_s * YanglongYanglongDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_YanglongYanglong_s * YanglongYanglongDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
