...
int YanglongYanglongDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_yanglongyanglong_list_size() < 0) || (table.e_yanglongyanglong_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_yanglongyanglong_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_projectid();
		}
		else
		{
			if (desc.has_m_projectid() < m_minId)
			{
				m_minId = desc.has_m_projectid();
			}
		}
		...
		auto pDesc = &m_astDescMap[desc.m_projectid()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_projectid());
		pDesc->read_from_pbmsg(desc);
