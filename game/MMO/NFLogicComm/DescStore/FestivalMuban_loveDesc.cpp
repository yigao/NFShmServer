...
int FestivalMuban_loveDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalMuban_loveDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_FestivalMuban_love table;
	...
	if ((table.e_festivalmuban_love_list_size() < 0) || (table.e_festivalmuban_love_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivalmuban_love_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_festivalmuban_love_list_size(); i++)
	{
		const proto_ff::E_FestivalMuban_love& desc = table.e_festivalmuban_love_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_festivalmuban_love_list_size());
