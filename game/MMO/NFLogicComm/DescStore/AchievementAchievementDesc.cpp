...
int AchievementAchievementDesc::Load(NFResDB *pDB)
{
	...
	for (int i = 0; i < (int)table.e_achievementachievement_list_size(); i++)
	{
		...
		uint64_t hashKey = desc.m_achievementid();
...
int AchievementAchievementDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		{
			CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_itemreward[j].m_id), result, "can't find the itemreward:{} in the  excel:item sheet:item", pDesc->m_itemreward[j].m_id);
		}
	}
	return result;
}

const proto_ff_s::E_AchievementAchievement_s * AchievementAchievementDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_AchievementAchievement_s * AchievementAchievementDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_AchievementAchievement_s *>((static_cast<const AchievementAchievementDesc*>(this))->GetDesc(id));
}

int AchievementAchievementDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_AchievementAchievement_s * AchievementAchievementDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_AchievementAchievement_s * AchievementAchievementDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
