#include "RuneStuntskillDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

RuneStuntskillDesc::RuneStuntskillDesc()
{
...
int RuneStuntskillDesc::CreateInit()
{
	return 0;
}

int RuneStuntskillDesc::ResumeInit()
{
	return 0;
}

int RuneStuntskillDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "RuneStuntskillDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_RuneStuntskill table;
	...
	if ((table.e_runestuntskill_list_size() < 0) || (table.e_runestuntskill_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_runestuntskill_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_runestuntskill_list_size(); i++)
	{
		const proto_ff::E_RuneStuntskill& desc = table.e_runestuntskill_list(i);
		...
		{
			m_minId = desc.has_m_skillid();
		}
		else
		{
			if (desc.has_m_skillid() < m_minId)
			{
				m_minId = desc.has_m_skillid();
			}
		}
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_runestuntskill_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int RuneStuntskillDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
