...
int FestivalMuban_severrankDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(BoxBoxDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_redpointbox), result, "can't find the redpointbox:{} in the  excel:box sheet:box", pDesc->m_redpointbox);
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_giftitemid), result, "can't find the giftitemid:{} in the  excel:item sheet:item", pDesc->m_giftitemid);
	}
	return result;
}
