#include "FestivalMuban_severrankDesc.h"
#include "BoxBoxDesc.h"
#include "ItemItemDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

IMPLEMENT_IDCREATE_WITHTYPE(FestivalMuban_severrankDesc, EOT_CONST_FESTIVAL_MUBAN_SEVERRANK_DESC_ID, NFShmObj)
...
int FestivalMuban_severrankDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalMuban_severrankDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_FestivalMuban_severrank table;
	...
	if ((table.e_festivalmuban_severrank_list_size() < 0) || (table.e_festivalmuban_severrank_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivalmuban_severrank_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_festivalmuban_severrank_list_size(); i++)
	{
		const proto_ff::E_FestivalMuban_severrank& desc = table.e_festivalmuban_severrank_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_festivalmuban_severrank_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int FestivalMuban_severrankDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		auto pDesc = &m_astDesc[i];
		CHECK_EXPR_MSG_RESULT((pDesc->m_redpointbox <= 0 || BoxBoxDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_redpointbox)), result, "can't find the redpointbox:{} in the  excel:box sheet:box", pDesc->m_redpointbox);
		CHECK_EXPR_MSG_RESULT((pDesc->m_giftitemid <= 0 || ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_giftitemid)), result, "can't find the giftitemid:{} in the  excel:item sheet:item", pDesc->m_giftitemid);
	}
	return result;
}
