...
int SkillSkillupDesc::Load(NFResDB *pDB)
{
	...
	proto_ff::Sheet_skillskillUp table;
	...
	if ((table.skillskillup_list_size() < 0) || (table.skillskillup_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.skillskillup_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.skillskillup_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.skillskillup_list_size(); i++)
	{
		const proto_ff::skillskillUp& desc = table.skillskillup_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.skillskillup_list_size());
...
const proto_ff_s::skillskillUp_s * SkillSkillupDesc::GetDesc(int id) const
{
...
proto_ff_s::skillskillUp_s * SkillSkillupDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::skillskillUp_s *>((static_cast<const SkillSkillupDesc*>(this))->GetDesc(id));
}
