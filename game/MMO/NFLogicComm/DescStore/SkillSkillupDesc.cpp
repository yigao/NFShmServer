...
int SkillSkillupDesc::Load(NFResDB *pDB)
{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "SkillSkillupDesc::Load() strFileName = {}", GetFileName());
	...
	if ((table.e_skillskillup_list_size() < 0) || (table.e_skillskillup_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_skillskillup_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_skillskillup_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.e_skillskillup_list_size(); i++)
	{
		const proto_ff::E_SkillSkillup& desc = table.e_skillskillup_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_skillskillup_list_size());
...
const proto_ff_s::E_SkillSkillup_s * SkillSkillupDesc::GetDesc(int id) const
{
...
proto_ff_s::E_SkillSkillup_s * SkillSkillupDesc::GetDesc(int id)
{
