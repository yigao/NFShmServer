...
int AvatarSkillDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "AvatarSkillDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_AvatarSkill table;
	...
	if ((table.e_avatarskill_list_size() < 0) || (table.e_avatarskill_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_avatarskill_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_avatarskill_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.e_avatarskill_list_size(); i++)
	{
		const proto_ff::E_AvatarSkill& desc = table.e_avatarskill_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_avatarskill_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int AvatarSkillDesc::CheckWhenAllDataLoaded()
{
	return 0;
}

const proto_ff_s::E_AvatarSkill_s * AvatarSkillDesc::GetDesc(int id) const
{
...
proto_ff_s::E_AvatarSkill_s * AvatarSkillDesc::GetDesc(int id)
{
