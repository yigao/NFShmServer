...
FestivalMuban_donate_singleDesc::~FestivalMuban_donate_singleDesc()
{
}

int FestivalMuban_donate_singleDesc::CreateInit()
{
	return Initialize();
}

int FestivalMuban_donate_singleDesc::ResumeInit()
{
	return 0;
}

int FestivalMuban_donate_singleDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalMuban_donate_singleDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_FestivalMuban_donate_single table;
	...
	if ((table.e_festivalmuban_donate_single_list_size() < 0) || (table.e_festivalmuban_donate_single_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivalmuban_donate_single_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_festivalmuban_donate_single_list_size(); i++)
	{
		const proto_ff::E_FestivalMuban_donate_single& desc = table.e_festivalmuban_donate_single_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_festivalmuban_donate_single_list_size());
...
proto_ff_s::E_FestivalMuban_donate_single_s * FestivalMuban_donate_singleDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_FestivalMuban_donate_single_s *>((static_cast<const FestivalMuban_donate_singleDesc*>(this))->GetDesc(id));
}

int FestivalMuban_donate_singleDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_FestivalMuban_donate_single_s * FestivalMuban_donate_singleDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_FestivalMuban_donate_single_s * FestivalMuban_donate_singleDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
