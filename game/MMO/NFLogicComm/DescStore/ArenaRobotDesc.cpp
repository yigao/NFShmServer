...
int ArenaRobotDesc::Load(NFResDB *pDB)
{
	...
	if ((table.arenarobot_list_size() < 0) || (table.arenarobot_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.arenarobot_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.arenarobot_list_size());
	...
	for (int i = 0; i < (int)table.arenarobot_list_size(); i++)
	{
		...
		auto pIndex = m_astDescMap.Insert(desc.arenaid());
		CHECK_EXPR(pIndex, -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.arenaid());
		*pIndex = i;
