...
int ArenaRobotDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_arenarobot_list_size() < 0) || (table.e_arenarobot_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_arenarobot_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_arenarobot_list_size());
	...
	for (int i = 0; i < (int)table.e_arenarobot_list_size(); i++)
	{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_arenaid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_arenaid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_arenaid());
		uint64_t hashKey = desc.m_arenaid();
...
const proto_ff_s::E_ArenaRobot_s * ArenaRobotDesc::GetDesc(int id) const
{
...
proto_ff_s::E_ArenaRobot_s * ArenaRobotDesc::GetDesc(int id)
{
