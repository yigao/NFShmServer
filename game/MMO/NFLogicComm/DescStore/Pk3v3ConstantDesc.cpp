...
int Pk3v3ConstantDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "Pk3v3ConstantDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_Pk3v3Constant table;
	...
	if ((table.e_pk3v3constant_list_size() < 0) || (table.e_pk3v3constant_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_pk3v3constant_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_pk3v3constant_list_size(); i++)
	{
		const proto_ff::E_Pk3v3Constant& desc = table.e_pk3v3constant_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_pk3v3constant_list_size());
