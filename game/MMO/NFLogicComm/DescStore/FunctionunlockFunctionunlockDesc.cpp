...
int FunctionunlockFunctionunlockDesc::Load(NFResDB *pDB)
{
	...
	if ((table.functionunlockfunctionunlock_list_size() < 0) || (table.functionunlockfunctionunlock_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.functionunlockfunctionunlock_list_size());
		return -2;
	}

	m_astDesc.resize(table.functionunlockfunctionunlock_list_size());
