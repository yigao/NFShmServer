...
int FunctionunlockFunctionunlockDesc::Load(NFResDB *pDB)
{
	...
	if ((table.functionunlockfunctionunlock_list_size() < 0) || (table.functionunlockfunctionunlock_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.functionunlockfunctionunlock_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.functionunlockfunctionunlock_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.functionunlockfunctionunlock_list_size(); i++)
	{
...
{
	return const_cast<proto_ff_s::functionunlockfunctionUnlock_s *>((static_cast<const FunctionunlockFunctionunlockDesc*>(this))->GetDesc(id));
}
