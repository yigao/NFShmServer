...
int BossBosstypeDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_bossbosstype_list_size() < 0) || (table.e_bossbosstype_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_bossbosstype_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_bossbosstype_list_size());
	...
	for (int i = 0; i < (int)table.e_bossbosstype_list_size(); i++)
	{
		...
		if (desc.has_m_bosstype() == false && desc.ByteSize() == 0)
		{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_bosstype());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_bosstype(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_bosstype());
		uint64_t hashKey = desc.m_bosstype();
...
const proto_ff_s::E_BossBosstype_s * BossBosstypeDesc::GetDesc(int id) const
{
...
proto_ff_s::E_BossBosstype_s * BossBosstypeDesc::GetDesc(int id)
{
