...
int CollectAttributetpyeDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "CollectAttributetpyeDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_CollectAttributetpye table;
	...
	if ((table.e_collectattributetpye_list_size() < 0) || (table.e_collectattributetpye_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_collectattributetpye_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_collectattributetpye_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.e_collectattributetpye_list_size(); i++)
	{
		const proto_ff::E_CollectAttributetpye& desc = table.e_collectattributetpye_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_collectattributetpye_list_size());
...
const proto_ff_s::E_CollectAttributetpye_s * CollectAttributetpyeDesc::GetDesc(int id) const
{
...
proto_ff_s::E_CollectAttributetpye_s * CollectAttributetpyeDesc::GetDesc(int id)
{
