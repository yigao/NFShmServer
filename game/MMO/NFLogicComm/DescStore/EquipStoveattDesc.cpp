...
IMPLEMENT_IDCREATE_WITHTYPE(EquipStoveattDesc, EOT_CONST_EQUIP_STOVEATT_DESC_ID, NFShmObj)

EquipStoveattDesc::EquipStoveattDesc():NFIDescStore()
{
...
int EquipStoveattDesc::Load(NFResDB *pDB)
{
	...
	if ((table.equipstoveatt_list_size() < 0) || (table.equipstoveatt_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.equipstoveatt_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.equipstoveatt_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.equipstoveatt_list_size(); i++)
	{
		const proto_ff::equipstoveatt& desc = table.equipstoveatt_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.equipstoveatt_list_size());
...
const proto_ff_s::equipstoveatt_s * EquipStoveattDesc::GetDesc(int id) const
{
...
proto_ff_s::equipstoveatt_s * EquipStoveattDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::equipstoveatt_s *>((static_cast<const EquipStoveattDesc*>(this))->GetDesc(id));
}
