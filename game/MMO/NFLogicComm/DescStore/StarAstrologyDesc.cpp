...
int StarAstrologyDesc::Load(NFResDB *pDB)
{
	...
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.starastrology_list_size());
		return -2;
	}
	...
	for (int i = 0; i < table.starastrology_list_size(); i++)
	{
		...
		if (desc.has_astrology() == false && desc.ByteSize() == 0)
		{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.starastrology_list_size());
...
const proto_ff_s::starastrology_s * StarAstrologyDesc::GetDesc(int id) const
{
