...
int WelfareLvrewardDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(BoxBoxDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_vipreward), result, "can't find the vipreward:{} in the  excel:box sheet:box", pDesc->m_vipreward);
		CHECK_EXPR_MSG_RESULT(VipVipDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_vip), result, "can't find the vip:{} in the  excel:vip sheet:vip", pDesc->m_vip);
