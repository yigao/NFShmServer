...
int HeadportraitHeadDesc::Load(NFResDB *pDB)
{
	...
	if ((table.headportraithead_list_size() < 0) || (table.headportraithead_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.headportraithead_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.headportraithead_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.headportraithead_list_size(); i++)
	{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.headportraithead_list_size());
...
const proto_ff_s::headPortraithead_s * HeadportraitHeadDesc::GetDesc(int id) const
{
...
proto_ff_s::headPortraithead_s * HeadportraitHeadDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::headPortraithead_s *>((static_cast<const HeadportraitHeadDesc*>(this))->GetDesc(id));
}
