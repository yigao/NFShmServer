...
int AncientgodAncientgodDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "AncientgodAncientgodDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_AncientgodAncientgod table;
	...
	if ((table.e_ancientgodancientgod_list_size() < 0) || (table.e_ancientgodancientgod_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_ancientgodancientgod_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_ancientgodancientgod_list_size(); i++)
	{
		const proto_ff::E_AncientgodAncientgod& desc = table.e_ancientgodancientgod_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_ancientgodancientgod_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int AncientgodAncientgodDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		{
			CHECK_EXPR_MSG_RESULT(AncientgodAttDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_fra[j].m_attid), result, "can't find the fra:{} in the  excel:ancientgod sheet:att", pDesc->m_fra[j].m_attid);
		}
	}
	return result;
}
