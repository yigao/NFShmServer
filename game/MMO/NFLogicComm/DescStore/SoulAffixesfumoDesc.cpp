#include "SoulAffixesfumoDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

SoulAffixesfumoDesc::SoulAffixesfumoDesc()
{
...
int SoulAffixesfumoDesc::CreateInit()
{
	return 0;
}

int SoulAffixesfumoDesc::ResumeInit()
{
	return 0;
}

int SoulAffixesfumoDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "SoulAffixesfumoDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_SoulAffixesfumo table;
	...
	if ((table.e_soulaffixesfumo_list_size() < 0) || (table.e_soulaffixesfumo_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_soulaffixesfumo_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_soulaffixesfumo_list_size(); i++)
	{
		const proto_ff::E_SoulAffixesfumo& desc = table.e_soulaffixesfumo_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_soulaffixesfumo_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int SoulAffixesfumoDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
