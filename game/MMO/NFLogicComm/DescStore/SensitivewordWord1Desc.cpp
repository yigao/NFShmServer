...
IMPLEMENT_IDCREATE_WITHTYPE(SensitivewordWord1Desc, EOT_CONST_SENSITIVEWORD_WORD1_DESC_ID, NFShmObj)
...
int SensitivewordWord1Desc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "SensitivewordWord1Desc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_SensitivewordWord1 table;
	...
	if ((table.e_sensitivewordword1_list_size() < 0) || (table.e_sensitivewordword1_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_sensitivewordword1_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_sensitivewordword1_list_size(); i++)
	{
		const proto_ff::E_SensitivewordWord1& desc = table.e_sensitivewordword1_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_sensitivewordword1_list_size());
