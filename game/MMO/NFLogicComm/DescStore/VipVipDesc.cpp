...
int VipVipDesc::Load(NFResDB *pDB)
{
	...
	if ((table.vipvip_list_size() < 0) || (table.vipvip_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.vipvip_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.vipvip_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.vipvip_list_size(); i++)
	{
		const proto_ff::vipvip& desc = table.vipvip_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.vipvip_list_size());
...
const proto_ff_s::vipvip_s * VipVipDesc::GetDesc(int id) const
{
...
proto_ff_s::vipvip_s * VipVipDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::vipvip_s *>((static_cast<const VipVipDesc*>(this))->GetDesc(id));
}
