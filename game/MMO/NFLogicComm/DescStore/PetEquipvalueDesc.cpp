...
IMPLEMENT_IDCREATE_WITHTYPE(PetEquipvalueDesc, EOT_CONST_PET_EQUIPVALUE_DESC_ID, NFShmObj)
...
int PetEquipvalueDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "PetEquipvalueDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_PetEquipvalue table;
	...
	if ((table.e_petequipvalue_list_size() < 0) || (table.e_petequipvalue_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_petequipvalue_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_petequipvalue_list_size(); i++)
	{
		const proto_ff::E_PetEquipvalue& desc = table.e_petequipvalue_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_petequipvalue_list_size());
