...
int MountHuakun_valueDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "MountHuakun_valueDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_MountHuakun_value table;
	...
	if ((table.e_mounthuakun_value_list_size() < 0) || (table.e_mounthuakun_value_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_mounthuakun_value_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_mounthuakun_value_list_size(); i++)
	{
		const proto_ff::E_MountHuakun_value& desc = table.e_mounthuakun_value_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_mounthuakun_value_list_size());
