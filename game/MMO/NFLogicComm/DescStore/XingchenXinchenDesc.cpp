...
int XingchenXinchenDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_xingchenxinchen_list_size() < 0) || (table.e_xingchenxinchen_list_size() > (int)(m_astDesc.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_xingchenxinchen_list_size(); i++)
	{
		...
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_xcequip());
		pDesc->read_from_pbmsg(desc);
