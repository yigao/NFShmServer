...
int RandomnameWomanfirstDesc::Load(NFResDB *pDB)
{
	...
	if ((table.randomnamewomanfirst_list_size() < 0) || (table.randomnamewomanfirst_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.randomnamewomanfirst_list_size());
		return -2;
	}

	m_astDesc.resize(table.randomnamewomanfirst_list_size());
