...
int FestivalMuban_shopDesc::Load(NFResDB *pDB)
{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalMuban_shopDesc::Load() strFileName = {}", GetFileName());
	...
	if ((table.e_festivalmuban_shop_list_size() < 0) || (table.e_festivalmuban_shop_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivalmuban_shop_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_festivalmuban_shop_list_size(); i++)
	{
		const proto_ff::E_FestivalMuban_shop& desc = table.e_festivalmuban_shop_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_festivalmuban_shop_list_size());
