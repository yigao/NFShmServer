...
int AvatarFragmentDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_item), result, "can't find the item:{} in the  excel:item sheet:item", pDesc->m_item);
	}
	return result;
}

const proto_ff_s::E_AvatarFragment_s * AvatarFragmentDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_AvatarFragment_s * AvatarFragmentDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_AvatarFragment_s *>((static_cast<const AvatarFragmentDesc*>(this))->GetDesc(id));
}

int AvatarFragmentDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_AvatarFragment_s * AvatarFragmentDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_AvatarFragment_s * AvatarFragmentDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
