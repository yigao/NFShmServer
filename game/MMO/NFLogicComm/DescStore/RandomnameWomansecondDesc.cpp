...
int RandomnameWomansecondDesc::Load(NFResDB *pDB)
{
	...
	if ((table.randomnamewomansecond_list_size() < 0) || (table.randomnamewomansecond_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.randomnamewomansecond_list_size());
		return -2;
	}

	m_astDesc.resize(table.randomnamewomansecond_list_size());
