#include "RuneQuenchingDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

RuneQuenchingDesc::RuneQuenchingDesc()
{
...
int RuneQuenchingDesc::CreateInit()
{
	return 0;
}

int RuneQuenchingDesc::ResumeInit()
{
	return 0;
}

int RuneQuenchingDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "RuneQuenchingDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_RuneQuenching table;
	...
	if ((table.e_runequenching_list_size() < 0) || (table.e_runequenching_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_runequenching_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_runequenching_list_size(); i++)
	{
		const proto_ff::E_RuneQuenching& desc = table.e_runequenching_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_runequenching_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int RuneQuenchingDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
