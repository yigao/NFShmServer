...
int XingchenXcmagicalDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_xingchenxcmagical_list_size() < 0) || (table.e_xingchenxcmagical_list_size() > (int)(m_astDesc.max_size())))
	{
...
int XingchenXcmagicalDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_hhstar), result, "can't find the hhstar:{} in the  excel:item sheet:item", pDesc->m_hhstar);
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_hhact), result, "can't find the hhact:{} in the  excel:item sheet:item", pDesc->m_hhact);
	}
	return result;
}

const proto_ff_s::E_XingchenXcmagical_s * XingchenXcmagicalDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_XingchenXcmagical_s * XingchenXcmagicalDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_XingchenXcmagical_s *>((static_cast<const XingchenXcmagicalDesc*>(this))->GetDesc(id));
}

int XingchenXcmagicalDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_XingchenXcmagical_s * XingchenXcmagicalDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_XingchenXcmagical_s * XingchenXcmagicalDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
