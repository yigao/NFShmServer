...
IMPLEMENT_IDCREATE_WITHTYPE(EncyclopediaBeastDesc, EOT_CONST_ENCYCLOPEDIA_BEAST_DESC_ID, NFShmObj)
...
int EncyclopediaBeastDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "EncyclopediaBeastDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_EncyclopediaBeast table;
	...
	if ((table.e_encyclopediabeast_list_size() < 0) || (table.e_encyclopediabeast_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_encyclopediabeast_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_encyclopediabeast_list_size(); i++)
	{
		const proto_ff::E_EncyclopediaBeast& desc = table.e_encyclopediabeast_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_encyclopediabeast_list_size());
