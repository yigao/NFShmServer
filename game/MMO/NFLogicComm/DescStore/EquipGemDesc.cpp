...
int EquipGemDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_equipgem_list_size() < 0) || (table.e_equipgem_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_equipgem_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_equipgem_list_size());
	...
	for (int i = 0; i < (int)table.e_equipgem_list_size(); i++)
	{
		...
		auto iter = m_astDescMap.emplace_hint(desc.m_position(), i);
...
const proto_ff_s::E_EquipGem_s * EquipGemDesc::GetDesc(int id) const
{
...
proto_ff_s::E_EquipGem_s * EquipGemDesc::GetDesc(int id)
{
