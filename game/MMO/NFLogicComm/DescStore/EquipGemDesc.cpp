...
int EquipGemDesc::Load(NFResDB *pDB)
{
	...
	if ((table.equipgem_list_size() < 0) || (table.equipgem_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.equipgem_list_size());
		return -2;
	}

	m_astDesc.resize(table.equipgem_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.equipgem_list_size(); i++)
	{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.position());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.position(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.position());
		uint64_t hashKey = desc.position();
...
{
	return const_cast<proto_ff_s::equipgem_s *>((static_cast<const EquipGemDesc*>(this))->GetDesc(id));
}
