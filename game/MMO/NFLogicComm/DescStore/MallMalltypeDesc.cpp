...
int MallMalltypeDesc::Load(NFResDB *pDB)
{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "MallMalltypeDesc::Load() strFileName = {}", GetFileName());
	...
	if ((table.e_mallmalltype_list_size() < 0) || (table.e_mallmalltype_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_mallmalltype_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_mallmalltype_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.e_mallmalltype_list_size(); i++)
	{
		const proto_ff::E_MallMalltype& desc = table.e_mallmalltype_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_mallmalltype_list_size());
...
const proto_ff_s::E_MallMalltype_s * MallMalltypeDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_MallMalltype_s * MallMalltypeDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_MallMalltype_s *>((static_cast<const MallMalltypeDesc*>(this))->GetDesc(id));
}

int MallMalltypeDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_MallMalltype_s * MallMalltypeDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_MallMalltype_s * MallMalltypeDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
