...
int DupBplevelDesc::Load(NFResDB *pDB)
{
	...
	if ((table.dupbplevel_list_size() < 0) || (table.dupbplevel_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.dupbplevel_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.dupbplevel_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.dupbplevel_list_size(); i++)
	{
		const proto_ff::dupBPlevel& desc = table.dupbplevel_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.dupbplevel_list_size());
...
const proto_ff_s::dupBPlevel_s * DupBplevelDesc::GetDesc(int id) const
{
...
proto_ff_s::dupBPlevel_s * DupBplevelDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::dupBPlevel_s *>((static_cast<const DupBplevelDesc*>(this))->GetDesc(id));
}
