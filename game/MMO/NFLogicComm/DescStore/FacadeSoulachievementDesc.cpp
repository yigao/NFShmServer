...
int FacadeSoulachievementDesc::Load(NFResDB *pDB)
{
	...
	if ((table.facadesoulachievement_list_size() < 0) || (table.facadesoulachievement_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.facadesoulachievement_list_size());
		return -2;
	}

	m_astDesc.resize(table.facadesoulachievement_list_size());
