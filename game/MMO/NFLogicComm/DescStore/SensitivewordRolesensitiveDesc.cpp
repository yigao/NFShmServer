...
int SensitivewordRolesensitiveDesc::Load(NFResDB *pDB)
{
	...
	if ((table.sensitivewordrolesensitive_list_size() < 0) || (table.sensitivewordrolesensitive_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.sensitivewordrolesensitive_list_size());
		return -2;
	}

	m_astDesc.resize(table.sensitivewordrolesensitive_list_size());
