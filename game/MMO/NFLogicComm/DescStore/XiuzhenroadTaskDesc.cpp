...
int XiuzhenroadTaskDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_xiuzhenroadtask_list_size() < 0) || (table.e_xiuzhenroadtask_list_size() > (int)(m_astDesc.max_size())))
	{
...
int XiuzhenroadTaskDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_reward), result, "can't find the reward:{} in the  excel:item sheet:item", pDesc->m_reward);
	}
	return result;
}

const proto_ff_s::E_XiuzhenroadTask_s * XiuzhenroadTaskDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_XiuzhenroadTask_s * XiuzhenroadTaskDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_XiuzhenroadTask_s *>((static_cast<const XiuzhenroadTaskDesc*>(this))->GetDesc(id));
}

int XiuzhenroadTaskDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_XiuzhenroadTask_s * XiuzhenroadTaskDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_XiuzhenroadTask_s * XiuzhenroadTaskDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
