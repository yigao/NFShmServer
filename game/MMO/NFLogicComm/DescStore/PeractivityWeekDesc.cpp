...
int PeractivityWeekDesc::Load(NFResDB *pDB)
{
	...
	proto_ff::Sheet_perActivityweek table;
	...
	if ((table.peractivityweek_list_size() < 0) || (table.peractivityweek_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.peractivityweek_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.peractivityweek_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.peractivityweek_list_size(); i++)
	{
		const proto_ff::perActivityweek& desc = table.peractivityweek_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.peractivityweek_list_size());
...
const proto_ff_s::perActivityweek_s * PeractivityWeekDesc::GetDesc(int id) const
{
...
proto_ff_s::perActivityweek_s * PeractivityWeekDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::perActivityweek_s *>((static_cast<const PeractivityWeekDesc*>(this))->GetDesc(id));
}
