...
int DecorateDecorateDesc::Load(NFResDB *pDB)
{
	...
	if ((table.decoratedecorate_list_size() < 0) || (table.decoratedecorate_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.decoratedecorate_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.decoratedecorate_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.decoratedecorate_list_size(); i++)
	{
		const proto_ff::decoratedecorate& desc = table.decoratedecorate_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.decoratedecorate_list_size());
...
const proto_ff_s::decoratedecorate_s * DecorateDecorateDesc::GetDesc(int id) const
{
...
proto_ff_s::decoratedecorate_s * DecorateDecorateDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::decoratedecorate_s *>((static_cast<const DecorateDecorateDesc*>(this))->GetDesc(id));
}
