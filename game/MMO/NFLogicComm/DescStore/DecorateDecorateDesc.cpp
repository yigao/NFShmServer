...
int DecorateDecorateDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_decoratedecorate_list_size() < 0) || (table.e_decoratedecorate_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_decoratedecorate_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_decoratedecorate_list_size());
...
const proto_ff_s::E_DecorateDecorate_s * DecorateDecorateDesc::GetDesc(int id) const
{
...
proto_ff_s::E_DecorateDecorate_s * DecorateDecorateDesc::GetDesc(int id)
{
