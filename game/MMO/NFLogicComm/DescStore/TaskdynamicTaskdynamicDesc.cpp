...
int TaskdynamicTaskdynamicDesc::Load(NFResDB *pDB)
{
	...
	if ((table.taskdynamictaskdynamic_list_size() < 0) || (table.taskdynamictaskdynamic_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.taskdynamictaskdynamic_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.taskdynamictaskdynamic_list_size());
	...
	for (int i = 0; i < (int)table.taskdynamictaskdynamic_list_size(); i++)
	{
		...
		auto pIndex = m_astDescMap.Insert(desc.taskid());
		CHECK_EXPR(pIndex, -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.taskid());
		*pIndex = i;
