...
int HaloHaloDesc::Load(NFResDB *pDB)
{
	...
	if ((table.halohalo_list_size() < 0) || (table.halohalo_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.halohalo_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.halohalo_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.halohalo_list_size(); i++)
	{
		const proto_ff::halohalo& desc = table.halohalo_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.halohalo_list_size());
...
const proto_ff_s::halohalo_s * HaloHaloDesc::GetDesc(int id) const
{
...
proto_ff_s::halohalo_s * HaloHaloDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::halohalo_s *>((static_cast<const HaloHaloDesc*>(this))->GetDesc(id));
}
