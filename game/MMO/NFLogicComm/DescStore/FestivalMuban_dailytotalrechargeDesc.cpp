#include "FestivalMuban_dailytotalrechargeDesc.h"
#include "BoxBoxDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

IMPLEMENT_IDCREATE_WITHTYPE(FestivalMuban_dailytotalrechargeDesc, EOT_CONST_FESTIVAL_MUBAN_DAILYTOTALRECHARGE_DESC_ID, NFShmObj)
...
int FestivalMuban_dailytotalrechargeDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalMuban_dailytotalrechargeDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_FestivalMuban_dailytotalrecharge table;
	...
	if ((table.e_festivalmuban_dailytotalrecharge_list_size() < 0) || (table.e_festivalmuban_dailytotalrecharge_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivalmuban_dailytotalrecharge_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_festivalmuban_dailytotalrecharge_list_size(); i++)
	{
		const proto_ff::E_FestivalMuban_dailytotalrecharge& desc = table.e_festivalmuban_dailytotalrecharge_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_festivalmuban_dailytotalrecharge_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int FestivalMuban_dailytotalrechargeDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
