...
int TaskrewardTaskrewardDesc::Load(NFResDB *pDB)
{
	...
	if ((table.taskrewardtaskreward_list_size() < 0) || (table.taskrewardtaskreward_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.taskrewardtaskreward_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.taskrewardtaskreward_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.taskrewardtaskreward_list_size(); i++)
	{
		const proto_ff::taskrewardtaskreward& desc = table.taskrewardtaskreward_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.taskrewardtaskreward_list_size());
...
proto_ff_s::taskrewardtaskreward_s * TaskrewardTaskrewardDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::taskrewardtaskreward_s *>((static_cast<const TaskrewardTaskrewardDesc*>(this))->GetDesc(id));
}
