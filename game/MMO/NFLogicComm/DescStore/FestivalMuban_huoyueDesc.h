#pragma once

#include "NFServerComm/NFDescStorePlugin/NFIDescStore.h"
#include "NFComm/NFShmCore/NFShmMgr.h"
#include "NFLogicCommon/NFDescStoreTypeDefines.h"
#include "NFServerLogicMessage/festival_s.h"

#define MAX_FESTIVAL_MUBAN_HUOYUE_NUM 300

class FestivalMuban_huoyueDesc : public NFIDescStore
{
public:
	FestivalMuban_huoyueDesc(NFIPluginManager* pPluginManager);
	virtual ~FestivalMuban_huoyueDesc();
	const proto_ff_s::festivalmuban_huoyue_s* GetDesc(int id) const;
	proto_ff_s::festivalmuban_huoyue_s* GetDesc(int id);
	const NFShmHashMap<uint64_t, proto_ff_s::festivalmuban_huoyue_s, MAX_FESTIVAL_MUBAN_HUOYUE_NUM>* GetAllDesc() const { return &m_astDesc; }
	NFShmHashMap<uint64_t, proto_ff_s::festivalmuban_huoyue_s, MAX_FESTIVAL_MUBAN_HUOYUE_NUM>* GetAllDesc() { return &m_astDesc; }
public:
IMPL_RES_HASH_DESC(proto_ff_s::festivalmuban_huoyue_s, festivalmuban_huoyue, MAX_FESTIVAL_MUBAN_HUOYUE_NUM);
DECLARE_IDCREATE(FestivalMuban_huoyueDesc);
};
