...
int OfflineItemlistDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_offlineitemlist_list_size() < 0) || (table.e_offlineitemlist_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_offlineitemlist_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_offlineitemlist_list_size());
...
const proto_ff_s::E_OfflineItemlist_s * OfflineItemlistDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_OfflineItemlist_s * OfflineItemlistDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_OfflineItemlist_s *>((static_cast<const OfflineItemlistDesc*>(this))->GetDesc(id));
}

int OfflineItemlistDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_OfflineItemlist_s * OfflineItemlistDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_OfflineItemlist_s * OfflineItemlistDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
