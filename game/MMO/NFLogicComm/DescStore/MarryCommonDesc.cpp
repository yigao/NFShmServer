...
IMPLEMENT_IDCREATE_WITHTYPE_GLOBAL(MarryCommonDesc, EOT_CONST_MARRY_COMMON_DESC_ID, NFShmObj)

MarryCommonDesc::MarryCommonDesc():NFIDescStore()
{
...
const proto_ff_s::E_MarryCommon_s * MarryCommonDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_MarryCommon_s * MarryCommonDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_MarryCommon_s *>((static_cast<const MarryCommonDesc*>(this))->GetDesc(id));
}

int MarryCommonDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_MarryCommon_s * MarryCommonDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_MarryCommon_s * MarryCommonDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
