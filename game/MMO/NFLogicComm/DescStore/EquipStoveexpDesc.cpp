...
int EquipStoveexpDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_equipstoveexp_list_size() < 0) || (table.e_equipstoveexp_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_equipstoveexp_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_equipstoveexp_list_size());
...
const proto_ff_s::E_EquipStoveexp_s * EquipStoveexpDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_EquipStoveexp_s * EquipStoveexpDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_EquipStoveexp_s *>((static_cast<const EquipStoveexpDesc*>(this))->GetDesc(id));
}

int EquipStoveexpDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_EquipStoveexp_s * EquipStoveexpDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_EquipStoveexp_s * EquipStoveexpDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
