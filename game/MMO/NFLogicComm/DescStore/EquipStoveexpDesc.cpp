...
IMPLEMENT_IDCREATE_WITHTYPE(EquipStoveexpDesc, EOT_CONST_EQUIP_STOVEEXP_DESC_ID, NFShmObj)

EquipStoveexpDesc::EquipStoveexpDesc():NFIDescStore()
{
...
int EquipStoveexpDesc::Load(NFResDB *pDB)
{
	...
	if ((table.equipstoveexp_list_size() < 0) || (table.equipstoveexp_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.equipstoveexp_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.equipstoveexp_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.equipstoveexp_list_size(); i++)
	{
		const proto_ff::equipstoveexp& desc = table.equipstoveexp_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.equipstoveexp_list_size());
...
const proto_ff_s::equipstoveexp_s * EquipStoveexpDesc::GetDesc(int id) const
{
...
proto_ff_s::equipstoveexp_s * EquipStoveexpDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::equipstoveexp_s *>((static_cast<const EquipStoveexpDesc*>(this))->GetDesc(id));
}
