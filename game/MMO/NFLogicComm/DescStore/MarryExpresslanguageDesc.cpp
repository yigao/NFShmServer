...
IMPLEMENT_IDCREATE_WITHTYPE(MarryExpresslanguageDesc, EOT_CONST_MARRY_EXPRESSLANGUAGE_DESC_ID, NFShmObj)
...
int MarryExpresslanguageDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "MarryExpresslanguageDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_MarryExpresslanguage table;
	...
	if ((table.e_marryexpresslanguage_list_size() < 0) || (table.e_marryexpresslanguage_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_marryexpresslanguage_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_marryexpresslanguage_list_size(); i++)
	{
		const proto_ff::E_MarryExpresslanguage& desc = table.e_marryexpresslanguage_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_marryexpresslanguage_list_size());
