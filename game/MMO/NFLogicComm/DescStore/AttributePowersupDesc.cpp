...
int AttributePowersupDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_attributepowersup_list_size() < 0) || (table.e_attributepowersup_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_attributepowersup_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_powerdiff();
		}
		else
		{
			if (desc.has_m_powerdiff() < m_minId)
			{
				m_minId = desc.has_m_powerdiff();
			}
		}
		...
		auto pDesc = &m_astDescMap[desc.m_powerdiff()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_powerdiff());
		pDesc->read_from_pbmsg(desc);
