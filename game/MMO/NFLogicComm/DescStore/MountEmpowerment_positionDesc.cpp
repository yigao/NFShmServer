...
int MountEmpowerment_positionDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "MountEmpowerment_positionDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_MountEmpowerment_position table;
	...
	if ((table.e_mountempowerment_position_list_size() < 0) || (table.e_mountempowerment_position_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_mountempowerment_position_list_size());
		return -2;
	}
	...
	for (int i = 0; i < (int)table.e_mountempowerment_position_list_size(); i++)
	{
		...
		if (m_astDescMap.find(desc.m_empowermentslot()) != m_astDescMap.end())
		{
			if (IsReloading())
			{
				auto pDesc = GetDesc(desc.m_empowermentslot());
				NF_ASSERT_MSG(pDesc, "the desc:{} Reload, GetDesc Failed!, id:{}", GetClassName(), desc.m_empowermentslot());
				pDesc->read_from_pbmsg(desc);
			}
			else
			{
				NFLogError(NF_LOG_SYSTEMLOG, 0, "the desc:{} id:{} exist", GetClassName(), desc.m_empowermentslot());
			}
			continue;
		}
		m_astDesc.push_back();
		...
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_empowermentslot());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_empowermentslot(), curIndex);
		CHECK_EXPR_ASSERT(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_empowermentslot());
		CHECK_EXPR_ASSERT(GetDesc(desc.m_empowermentslot()) == pDesc, -1, "GetDesc != pDesc, id:{}", desc.m_empowermentslot());
	}

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_mountempowerment_position_list_size());
