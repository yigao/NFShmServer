...
IMPLEMENT_IDCREATE_WITHTYPE(FacadeSoullvDesc, EOT_CONST_FACADE_SOULLV_DESC_ID, NFShmObj)

FacadeSoullvDesc::FacadeSoullvDesc():NFIDescStore()
{
...
int FacadeSoullvDesc::Load(NFResDB *pDB)
{
	...
	if ((table.facadesoullv_list_size() < 0) || (table.facadesoullv_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.facadesoullv_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.facadesoullv_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.facadesoullv_list_size(); i++)
	{
		const proto_ff::facadesoulLv& desc = table.facadesoullv_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.facadesoullv_list_size());
...
const proto_ff_s::facadesoulLv_s * FacadeSoullvDesc::GetDesc(int id) const
{
...
proto_ff_s::facadesoulLv_s * FacadeSoullvDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::facadesoulLv_s *>((static_cast<const FacadeSoullvDesc*>(this))->GetDesc(id));
}
