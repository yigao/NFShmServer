...
int RandomnameWomanthirdDesc::Load(NFResDB *pDB)
{
	...
	if ((table.randomnamewomanthird_list_size() < 0) || (table.randomnamewomanthird_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.randomnamewomanthird_list_size());
		return -2;
	}

	m_astDesc.resize(table.randomnamewomanthird_list_size());
