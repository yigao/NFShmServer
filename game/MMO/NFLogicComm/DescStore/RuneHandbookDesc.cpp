#include "RuneHandbookDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

RuneHandbookDesc::RuneHandbookDesc()
{
...
int RuneHandbookDesc::CreateInit()
{
	return 0;
}

int RuneHandbookDesc::ResumeInit()
{
	return 0;
}

int RuneHandbookDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "RuneHandbookDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_RuneHandbook table;
	...
	if ((table.e_runehandbook_list_size() < 0) || (table.e_runehandbook_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_runehandbook_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_runehandbook_list_size(); i++)
	{
		const proto_ff::E_RuneHandbook& desc = table.e_runehandbook_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_runehandbook_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int RuneHandbookDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
