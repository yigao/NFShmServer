...
int FestivalMuban_collectwordDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_festivalmuban_collectword_list_size() < 0) || (table.e_festivalmuban_collectword_list_size() > (int)(m_astDesc.max_size())))
	{
...
int FestivalMuban_collectwordDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_exchange_id), result, "can't find the exchange_id:{} in the  excel:item sheet:item", pDesc->m_exchange_id);
		for(int j = 0; j < (int)pDesc->m_cost.size(); j++)
		{
			CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_cost[j].m_id), result, "can't find the cost:{} in the  excel:item sheet:item", pDesc->m_cost[j].m_id);
		}
	}
	return result;
}

const proto_ff_s::E_FestivalMuban_collectword_s * FestivalMuban_collectwordDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_FestivalMuban_collectword_s * FestivalMuban_collectwordDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_FestivalMuban_collectword_s *>((static_cast<const FestivalMuban_collectwordDesc*>(this))->GetDesc(id));
}

int FestivalMuban_collectwordDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_FestivalMuban_collectword_s * FestivalMuban_collectwordDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_FestivalMuban_collectword_s * FestivalMuban_collectwordDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
