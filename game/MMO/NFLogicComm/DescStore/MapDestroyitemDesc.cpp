...
MapDestroyitemDesc::MapDestroyitemDesc(NFIPluginManager* pPluginManager):NFIDescStore(pPluginManager)
{
...
int MapDestroyitemDesc::Load(NFResDB *pDB)
{
	...
	if ((table.mapdestroyitem_list_size() < 0) || (table.mapdestroyitem_list_size() > (int)(m_astDesc.GetSize())))
	{
		NFLogError(NF_LOG_COMM_LOGIC_PLUGIN, 0, "Invalid TotalNum:{}", table.mapdestroyitem_list_size());
		return -2;
	}
	...
	NFLogTrace(NF_LOG_COMM_LOGIC_PLUGIN, 0, "load {}, num={}", iRet, table.mapdestroyitem_list_size());
	return 0;
...
const proto_ff_s::mapdestroyitem_s * MapDestroyitemDesc::CetDesc(int id) const
{
