...
int MapDestroyitemDesc::Load(NFResDB *pDB)
{
	...
	if ((table.mapdestroyitem_list_size() < 0) || (table.mapdestroyitem_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.mapdestroyitem_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.mapdestroyitem_list_size());
	...
	for (int i = 0; i < (int)table.mapdestroyitem_list_size(); i++)
	{
		...
		auto pIndex = m_astDescMap.Insert(desc.destroyitemid());
		CHECK_EXPR(pIndex, -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.destroyitemid());
		*pIndex = i;
