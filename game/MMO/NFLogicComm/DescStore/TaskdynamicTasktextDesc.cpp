...
int TaskdynamicTasktextDesc::Load(NFResDB *pDB)
{
	...
	if ((table.taskdynamictasktext_list_size() < 0) || (table.taskdynamictasktext_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.taskdynamictasktext_list_size());
		return -2;
	}

	m_astDesc.resize(table.taskdynamictasktext_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.taskdynamictasktext_list_size(); i++)
	{
		const proto_ff::taskdynamictasktext& desc = table.taskdynamictasktext_list(i);
		if (desc.has_textid() == false && desc.ByteSize() == 0)
		{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.textid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.textid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.textid());
		uint64_t hashKey = desc.textid();
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.taskdynamictasktext_list_size());
...
const proto_ff_s::taskdynamictasktext_s * TaskdynamicTasktextDesc::GetDesc(int id) const
{
...
{
	return const_cast<proto_ff_s::taskdynamictasktext_s *>((static_cast<const TaskdynamicTasktextDesc*>(this))->GetDesc(id));
}
