...
IMPLEMENT_IDCREATE_WITHTYPE_GLOBAL(EncyclopediaHandbooklv_attrDesc, EOT_CONST_ENCYCLOPEDIA_HANDBOOKLV_ATTR_DESC_ID, NFShmObj)

EncyclopediaHandbooklv_attrDesc::EncyclopediaHandbooklv_attrDesc():NFIDescStore()
{
...
const proto_ff_s::E_EncyclopediaHandbooklv_attr_s * EncyclopediaHandbooklv_attrDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_EncyclopediaHandbooklv_attr_s * EncyclopediaHandbooklv_attrDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_EncyclopediaHandbooklv_attr_s *>((static_cast<const EncyclopediaHandbooklv_attrDesc*>(this))->GetDesc(id));
}

int EncyclopediaHandbooklv_attrDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_EncyclopediaHandbooklv_attr_s * EncyclopediaHandbooklv_attrDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_EncyclopediaHandbooklv_attr_s * EncyclopediaHandbooklv_attrDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
