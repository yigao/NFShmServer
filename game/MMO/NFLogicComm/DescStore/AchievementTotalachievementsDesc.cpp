#include "AchievementTotalachievementsDesc.h"
...
int AchievementTotalachievementsDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "AchievementTotalachievementsDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_AchievementTotalachievements table;
	...
	if ((table.e_achievementtotalachievements_list_size() < 0) || (table.e_achievementtotalachievements_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_achievementtotalachievements_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_achievementtotalachievements_list_size(); i++)
	{
		const proto_ff::E_AchievementTotalachievements& desc = table.e_achievementtotalachievements_list(i);
		if (desc.has_m_totalsum() == false && desc.ByteSize() == 0)
		{
			NFLogError(NF_LOG_SYSTEMLOG, 0, "the desc no value, {}", desc.Utf8DebugString());
			continue;
		}
		//NFLogTrace(NF_LOG_SYSTEMLOG, 0, "{}", desc.Utf8DebugString());
		if (m_astDescMap.find(desc.m_totalsum()) != m_astDescMap.end())
		{
			if (IsReloading())
			{
				auto pDesc = GetDesc(desc.m_totalsum());
				NF_ASSERT_MSG(pDesc, "the desc:{} Reload, GetDesc Failed!, id:{}", GetClassName(), desc.m_totalsum());
				pDesc->read_from_pbmsg(desc);
			}
			else
			{
				NFLogError(NF_LOG_SYSTEMLOG, 0, "the desc:{} id:{} exist", GetClassName(), desc.m_totalsum());
			}
			continue;
		}
		m_astDesc.push_back();
		...
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_totalsum());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_totalsum(), curIndex);
		CHECK_EXPR_ASSERT(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_totalsum());
		uint64_t hashKey = desc.m_totalsum();
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_achievementtotalachievements_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int AchievementTotalachievementsDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		auto pDesc = &m_astDesc[i];
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_totalitems), result, "can't find the totalitems:{} in the  excel:item sheet:item", pDesc->m_totalitems);
	}
	return result;
}
