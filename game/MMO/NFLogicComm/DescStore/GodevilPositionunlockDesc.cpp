...
int GodevilPositionunlockDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_godevilpositionunlock_list_size() < 0) || (table.e_godevilpositionunlock_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_godevilpositionunlock_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_position();
		}
		else
		{
			if (desc.has_m_position() < m_minId)
			{
				m_minId = desc.has_m_position();
			}
		}
