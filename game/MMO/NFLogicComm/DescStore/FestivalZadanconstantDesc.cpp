...
IMPLEMENT_IDCREATE_WITHTYPE(FestivalZadanconstantDesc, EOT_CONST_FESTIVAL_ZADANCONSTANT_DESC_ID, NFShmObj)

FestivalZadanconstantDesc::FestivalZadanconstantDesc():NFIDescStore()
{
...
int FestivalZadanconstantDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_festivalzadanconstant_list_size() < 0) || (table.e_festivalzadanconstant_list_size() > (int)(m_astDesc.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_festivalzadanconstant_list_size(); i++)
	{
		...
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_period());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_period(), curIndex);
		CHECK_EXPR_ASSERT(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_period());
		uint64_t hashKey = desc.m_period();
...
int FestivalZadanconstantDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_costitem), result, "can't find the costitem:{} in the  excel:item sheet:item", pDesc->m_costitem);
	}
	return result;
}

const proto_ff_s::E_FestivalZadanconstant_s * FestivalZadanconstantDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_FestivalZadanconstant_s * FestivalZadanconstantDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_FestivalZadanconstant_s *>((static_cast<const FestivalZadanconstantDesc*>(this))->GetDesc(id));
}

int FestivalZadanconstantDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_FestivalZadanconstant_s * FestivalZadanconstantDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_FestivalZadanconstant_s * FestivalZadanconstantDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
