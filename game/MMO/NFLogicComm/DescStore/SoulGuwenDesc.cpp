#include "SoulGuwenDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

SoulGuwenDesc::SoulGuwenDesc()
{
...
int SoulGuwenDesc::CreateInit()
{
	return 0;
}

int SoulGuwenDesc::ResumeInit()
{
	return 0;
}

int SoulGuwenDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "SoulGuwenDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_SoulGuwen table;
	...
	if ((table.e_soulguwen_list_size() < 0) || (table.e_soulguwen_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_soulguwen_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_soulguwen_list_size(); i++)
	{
		const proto_ff::E_SoulGuwen& desc = table.e_soulguwen_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_soulguwen_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int SoulGuwenDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
