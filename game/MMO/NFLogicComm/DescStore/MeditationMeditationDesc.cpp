...
int MeditationMeditationDesc::Load(NFResDB *pDB)
{
	...
	if ((table.meditationmeditation_list_size() < 0) || (table.meditationmeditation_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.meditationmeditation_list_size());
		return -2;
	}

	m_astDesc.resize(table.meditationmeditation_list_size());
