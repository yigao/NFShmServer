...
const proto_ff_s::E_MeditationMeditation_s * MeditationMeditationDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_MeditationMeditation_s * MeditationMeditationDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_MeditationMeditation_s *>((static_cast<const MeditationMeditationDesc*>(this))->GetDesc(id));
}

int MeditationMeditationDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_MeditationMeditation_s * MeditationMeditationDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_MeditationMeditation_s * MeditationMeditationDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
