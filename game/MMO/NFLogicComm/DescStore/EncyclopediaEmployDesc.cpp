...
int EncyclopediaEmployDesc::Load(NFResDB *pDB)
{
	...
	for (int i = 0; i < (int)table.e_encyclopediaemploy_list_size(); i++)
	{
		...
		uint64_t hashKey = desc.m_employid();
...
int EncyclopediaEmployDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(PetDisplayDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_itemid) || AvatarChangeDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_itemid), result, "can't find the itemid:{} in the  excel:pet sheet:display or  excel:avatar sheet:change", pDesc->m_itemid);
	}
	return result;
}
