...
int XingchenXctpaddiDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(AttributeAttributeDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_attid), result, "can't find the attid:{} in the  excel:attribute sheet:attribute", pDesc->m_attid);
	}
	return result;
}
