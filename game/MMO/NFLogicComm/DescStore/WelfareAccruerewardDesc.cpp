#include "WelfareAccruerewardDesc.h"
...
int WelfareAccruerewardDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "WelfareAccruerewardDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_WelfareAccruereward table;
	...
	if ((table.e_welfareaccruereward_list_size() < 0) || (table.e_welfareaccruereward_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_welfareaccruereward_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_welfareaccruereward_list_size(); i++)
	{
		const proto_ff::E_WelfareAccruereward& desc = table.e_welfareaccruereward_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_welfareaccruereward_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int WelfareAccruerewardDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		auto pDesc = &m_astDesc[i];
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_reward) || EquipEquipDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_reward), result, "can't find the reward:{} in the  excel:item sheet:item or  excel:equip sheet:equip", pDesc->m_reward);
	}
	return result;
}
