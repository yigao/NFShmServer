...
int TaskrewardTasktypeDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_taskrewardtasktype_list_size() < 0) || (table.e_taskrewardtasktype_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_taskrewardtasktype_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_taskrewardtasktype_list_size());
	...
	for (int i = 0; i < (int)table.e_taskrewardtasktype_list_size(); i++)
	{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_tasktype());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_tasktype(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_tasktype());
		uint64_t hashKey = desc.m_tasktype();
...
const proto_ff_s::E_TaskrewardTasktype_s * TaskrewardTasktypeDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_TaskrewardTasktype_s * TaskrewardTasktypeDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_TaskrewardTasktype_s *>((static_cast<const TaskrewardTasktypeDesc*>(this))->GetDesc(id));
}

int TaskrewardTasktypeDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_TaskrewardTasktype_s * TaskrewardTasktypeDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_TaskrewardTasktype_s * TaskrewardTasktypeDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
