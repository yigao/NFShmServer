...
int TaskrewardTasktypeDesc::Load(NFResDB *pDB)
{
	...
	if ((table.taskrewardtasktype_list_size() < 0) || (table.taskrewardtasktype_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.taskrewardtasktype_list_size());
		return -2;
	}

	m_astDesc.resize(table.taskrewardtasktype_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.taskrewardtasktype_list_size(); i++)
	{
		const proto_ff::taskrewardtasktype& desc = table.taskrewardtasktype_list(i);
		if (desc.has_tasktype() == false && desc.ByteSize() == 0)
		{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.tasktype());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.tasktype(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.tasktype());
		uint64_t hashKey = desc.tasktype();
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.taskrewardtasktype_list_size());
...
const proto_ff_s::taskrewardtasktype_s * TaskrewardTasktypeDesc::GetDesc(int id) const
{
...
proto_ff_s::taskrewardtasktype_s * TaskrewardTasktypeDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::taskrewardtasktype_s *>((static_cast<const TaskrewardTasktypeDesc*>(this))->GetDesc(id));
}
