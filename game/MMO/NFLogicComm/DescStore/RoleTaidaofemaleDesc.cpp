...
int RoleTaidaofemaleDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_roletaidaofemale_list_size() < 0) || (table.e_roletaidaofemale_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_roletaidaofemale_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_roletaidaofemale_list_size());
...
const proto_ff_s::E_RoleTaidaofemale_s * RoleTaidaofemaleDesc::GetDesc(int id) const
{
...
proto_ff_s::E_RoleTaidaofemale_s * RoleTaidaofemaleDesc::GetDesc(int id)
{
