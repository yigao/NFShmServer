...
int WelfareQifuDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_welfareqifu_list_size() < 0) || (table.e_welfareqifu_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_welfareqifu_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_constantid();
		}
		else
		{
			if (desc.has_m_constantid() < m_minId)
			{
				m_minId = desc.has_m_constantid();
			}
		}
