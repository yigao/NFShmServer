...
int PeractivityPeractivityDesc::Load(NFResDB *pDB)
{
	...
	if ((table.peractivityperactivity_list_size() < 0) || (table.peractivityperactivity_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.peractivityperactivity_list_size());
		return -2;
	}

	m_astDesc.resize(table.peractivityperactivity_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.peractivityperactivity_list_size(); i++)
	{
		const proto_ff::perActivityperActivity& desc = table.peractivityperactivity_list(i);
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.activeid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.activeid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.activeid());
		uint64_t hashKey = desc.activeid();
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.peractivityperactivity_list_size());
...
const proto_ff_s::perActivityperActivity_s * PeractivityPeractivityDesc::GetDesc(int id) const
{
...
proto_ff_s::perActivityperActivity_s * PeractivityPeractivityDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::perActivityperActivity_s *>((static_cast<const PeractivityPeractivityDesc*>(this))->GetDesc(id));
}
