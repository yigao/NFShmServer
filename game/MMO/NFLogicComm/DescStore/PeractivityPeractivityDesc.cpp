...
int PeractivityPeractivityDesc::Load(NFResDB *pDB)
{
	...
	if ((table.peractivityperactivity_list_size() < 0) || (table.peractivityperactivity_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.peractivityperactivity_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.peractivityperactivity_list_size());
	...
	for (int i = 0; i < (int)table.peractivityperactivity_list_size(); i++)
	{
		...
		auto pIndex = m_astDescMap.Insert(desc.activeid());
		CHECK_EXPR(pIndex, -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.activeid());
		*pIndex = i;
