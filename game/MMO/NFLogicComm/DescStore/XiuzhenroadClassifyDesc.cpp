...
int XiuzhenroadClassifyDesc::Load(NFResDB *pDB)
{
	...
	for (int i = 0; i < (int)table.e_xiuzhenroadclassify_list_size(); i++)
	{
		...
		uint64_t hashKey = desc.m_typeid();
...
int XiuzhenroadClassifyDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(BoxBoxDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_giftid), result, "can't find the giftid:{} in the  excel:box sheet:box", pDesc->m_giftid);
	}
	return result;
}
