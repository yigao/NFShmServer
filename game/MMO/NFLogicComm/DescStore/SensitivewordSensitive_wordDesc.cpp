...
int SensitivewordSensitive_wordDesc::Load(NFResDB *pDB)
{
	...
	if ((table.sensitivewordsensitive_word_list_size() < 0) || (table.sensitivewordsensitive_word_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.sensitivewordsensitive_word_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.sensitivewordsensitive_word_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.sensitivewordsensitive_word_list_size(); i++)
	{
...
const proto_ff_s::sensitivewordsensitive_word_s * SensitivewordSensitive_wordDesc::GetDesc(int id) const
{
...
{
	return const_cast<proto_ff_s::sensitivewordsensitive_word_s *>((static_cast<const SensitivewordSensitive_wordDesc*>(this))->GetDesc(id));
}
