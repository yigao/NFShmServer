...
int SensitivewordSensitive_wordDesc::Load(NFResDB *pDB)
{
	...
	if ((table.sensitivewordsensitive_word_list_size() < 0) || (table.sensitivewordsensitive_word_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.sensitivewordsensitive_word_list_size());
		return -2;
	}

	m_astDesc.resize(table.sensitivewordsensitive_word_list_size());
