...
int WelfareLogonDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_welfarelogon_list_size() < 0) || (table.e_welfarelogon_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_welfarelogon_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_days();
		}
		else
		{
			if (desc.has_m_days() < m_minId)
			{
				m_minId = desc.has_m_days();
			}
		}
		...
		auto pDesc = &m_astDescMap[desc.m_days()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_days());
		pDesc->read_from_pbmsg(desc);
