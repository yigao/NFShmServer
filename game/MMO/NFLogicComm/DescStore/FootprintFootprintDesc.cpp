...
int FootprintFootprintDesc::Load(NFResDB *pDB)
{
	...
	if ((table.footprintfootprint_list_size() < 0) || (table.footprintfootprint_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.footprintfootprint_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.footprintfootprint_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.footprintfootprint_list_size(); i++)
	{
		const proto_ff::footprintfootprint& desc = table.footprintfootprint_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.footprintfootprint_list_size());
...
const proto_ff_s::footprintfootprint_s * FootprintFootprintDesc::GetDesc(int id) const
{
...
proto_ff_s::footprintfootprint_s * FootprintFootprintDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::footprintfootprint_s *>((static_cast<const FootprintFootprintDesc*>(this))->GetDesc(id));
}
