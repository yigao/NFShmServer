...
ChatSpecialchatDesc::ChatSpecialchatDesc(NFIPluginManager* pPluginManager):NFIDescStore(pPluginManager)
{
...
int ChatSpecialchatDesc::Load(NFResDB *pDB)
{
	...
	if ((table.chatspecialchat_list_size() < 0) || (table.chatspecialchat_list_size() > (int)(m_astDesc.GetSize())))
	{
		NFLogError(NF_LOG_COMM_LOGIC_PLUGIN, 0, "Invalid TotalNum:{}", table.chatspecialchat_list_size());
		return -2;
	}

	for (int i = 0; i < table.chatspecialchat_list_size(); i++)
	{
	...
	NFLogTrace(NF_LOG_COMM_LOGIC_PLUGIN, 0, "load {}, num={}", iRet, table.chatspecialchat_list_size());
	return 0;
