...
IMPLEMENT_IDCREATE_WITHTYPE_GLOBAL(ChatSpecialchatDesc, EOT_CONST_CHAT_SPECIALCHAT_DESC_ID, NFShmObj)
...
int ChatSpecialchatDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_chatspecialchat_list_size() < 0) || (table.e_chatspecialchat_list_size() > (int)(m_astDesc.max_size())))
	{
...
const proto_ff_s::E_ChatSpecialchat_s * ChatSpecialchatDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_ChatSpecialchat_s * ChatSpecialchatDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_ChatSpecialchat_s *>((static_cast<const ChatSpecialchatDesc*>(this))->GetDesc(id));
}

int ChatSpecialchatDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_ChatSpecialchat_s * ChatSpecialchatDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_ChatSpecialchat_s * ChatSpecialchatDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
