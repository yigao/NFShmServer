...
int FestivalMuban_donateDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(BoxBoxDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_donatereward), result, "can't find the donatereward:{} in the  excel:box sheet:box", pDesc->m_donatereward);
		CHECK_EXPR_MSG_RESULT(ItemItemDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_donateitem), result, "can't find the donateitem:{} in the  excel:item sheet:item", pDesc->m_donateitem);
	}
	return result;
}
