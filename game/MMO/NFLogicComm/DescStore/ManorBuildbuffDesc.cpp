...
int ManorBuildbuffDesc::Load(NFResDB *pDB)
{
	...
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.manorbuildbuff_list_size());
		return -2;
	}
	...
	for (int i = 0; i < table.manorbuildbuff_list_size(); i++)
	{
		...
		if (desc.has_levelsum() == false && desc.ByteSize() == 0)
		{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.manorbuildbuff_list_size());
...
const proto_ff_s::manorbuildBuff_s * ManorBuildbuffDesc::GetDesc(int id) const
{
