...
int GuildWarreviveDesc::Load(NFResDB *pDB)
{
	...
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.guildwarrevive_list_size());
		return -2;
	}
	...
	for (int i = 0; i < table.guildwarrevive_list_size(); i++)
	{
		...
		if (desc.has_revivenum() == false && desc.ByteSize() == 0)
		{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.guildwarrevive_list_size());
