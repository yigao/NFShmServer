...
int XingchenXcguizhenDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		{
			CHECK_EXPR_MSG_RESULT(AttributeAttributeDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_gz[j].m_type), result, "can't find the gz:{} in the  excel:attribute sheet:attribute", pDesc->m_gz[j].m_type);
		}
		for(int j = 0; j < (int)pDesc->m_xx.size(); j++)
		{
			CHECK_EXPR_MSG_RESULT(AttributeAttributeDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_xx[j].m_type), result, "can't find the xx:{} in the  excel:attribute sheet:attribute", pDesc->m_xx[j].m_type);
		}
	}
	return result;
}
