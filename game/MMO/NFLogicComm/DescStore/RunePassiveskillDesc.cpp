#include "RunePassiveskillDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

RunePassiveskillDesc::RunePassiveskillDesc()
{
...
int RunePassiveskillDesc::CreateInit()
{
	return 0;
}

int RunePassiveskillDesc::ResumeInit()
{
	return 0;
}

int RunePassiveskillDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "RunePassiveskillDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_RunePassiveskill table;
	...
	if ((table.e_runepassiveskill_list_size() < 0) || (table.e_runepassiveskill_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_runepassiveskill_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_runepassiveskill_list_size(); i++)
	{
		const proto_ff::E_RunePassiveskill& desc = table.e_runepassiveskill_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_runepassiveskill_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int RunePassiveskillDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
