...
int GodrelicsConditionDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_godrelicscondition_list_size() < 0) || (table.e_godrelicscondition_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_godrelicscondition_list_size());
		return -2;
	}

	m_astDesc.resize(table.e_godrelicscondition_list_size());
...
const proto_ff_s::E_GodrelicsCondition_s * GodrelicsConditionDesc::GetDesc(int id) const
{
...
proto_ff_s::E_GodrelicsCondition_s * GodrelicsConditionDesc::GetDesc(int id)
{
