...
int MonsterMonsterDesc::Load(NFResDB *pDB)
{
	...
	if ((table.monstermonster_list_size() < 0) || (table.monstermonster_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.monstermonster_list_size());
		return -2;
	}

	m_astDesc.resize(table.monstermonster_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.monstermonster_list_size(); i++)
	{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.monsterid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.monsterid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.monsterid());
		uint64_t hashKey = desc.monsterid();
...
{
	return const_cast<proto_ff_s::monstermonster_s *>((static_cast<const MonsterMonsterDesc*>(this))->GetDesc(id));
}
