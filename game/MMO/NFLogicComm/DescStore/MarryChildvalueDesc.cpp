...
IMPLEMENT_IDCREATE_WITHTYPE(MarryChildvalueDesc, EOT_CONST_MARRY_CHILDVALUE_DESC_ID, NFShmObj)
...
int MarryChildvalueDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "MarryChildvalueDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_MarryChildvalue table;
	...
	if ((table.e_marrychildvalue_list_size() < 0) || (table.e_marrychildvalue_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_marrychildvalue_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_marrychildvalue_list_size(); i++)
	{
		const proto_ff::E_MarryChildvalue& desc = table.e_marrychildvalue_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_marrychildvalue_list_size());
