#include "ShenequipTalentDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

ShenequipTalentDesc::ShenequipTalentDesc()
{
...
int ShenequipTalentDesc::CreateInit()
{
	return 0;
}

int ShenequipTalentDesc::ResumeInit()
{
	return 0;
}

int ShenequipTalentDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "ShenequipTalentDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_ShenequipTalent table;
	...
	if ((table.e_shenequiptalent_list_size() < 0) || (table.e_shenequiptalent_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_shenequiptalent_list_size());
		return -2;
	}

	m_minId = INVALID_ID;
	for (int i = 0; i < (int)table.e_shenequiptalent_list_size(); i++)
	{
		const proto_ff::E_ShenequipTalent& desc = table.e_shenequiptalent_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_shenequiptalent_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int ShenequipTalentDesc::CheckWhenAllDataLoaded()
{
	return 0;
}
