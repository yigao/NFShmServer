...
int MoneyrewardMoneyrewardDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_moneyrewardmoneyreward_list_size() < 0) || (table.e_moneyrewardmoneyreward_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_moneyrewardmoneyreward_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_taskreward();
		}
		else
		{
			if (desc.has_m_taskreward() < m_minId)
			{
				m_minId = desc.has_m_taskreward();
			}
		}
		...
		auto pDesc = &m_astDescMap[desc.m_taskreward()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_taskreward());
		pDesc->read_from_pbmsg(desc);
