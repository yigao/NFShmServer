...
int BackBackDesc::Load(NFResDB *pDB)
{
	...
	if ((table.backback_list_size() < 0) || (table.backback_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.backback_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.backback_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.backback_list_size(); i++)
	{
		const proto_ff::backback& desc = table.backback_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.backback_list_size());
...
const proto_ff_s::backback_s * BackBackDesc::GetDesc(int id) const
{
...
proto_ff_s::backback_s * BackBackDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::backback_s *>((static_cast<const BackBackDesc*>(this))->GetDesc(id));
}
