...
int RankingButtonDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_rankingbutton_list_size() < 0) || (table.e_rankingbutton_list_size() > (int)(m_astDesc.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_rankingbutton_list_size(); i++)
	{
		...
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_buttonid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_buttonid(), curIndex);
		CHECK_EXPR_ASSERT(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_buttonid());
		uint64_t hashKey = desc.m_buttonid();
...
const proto_ff_s::E_RankingButton_s * RankingButtonDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_RankingButton_s * RankingButtonDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_RankingButton_s *>((static_cast<const RankingButtonDesc*>(this))->GetDesc(id));
}

int RankingButtonDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_RankingButton_s * RankingButtonDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_RankingButton_s * RankingButtonDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
