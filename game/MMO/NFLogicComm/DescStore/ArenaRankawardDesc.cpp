...
int ArenaRankawardDesc::Load(NFResDB *pDB)
{
	...
	if ((table.arenarankaward_list_size() < 0) || (table.arenarankaward_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.arenarankaward_list_size());
		return -2;
	}

	m_astDesc.resize(table.arenarankaward_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.arenarankaward_list_size(); i++)
	{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.prizeid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.prizeid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.prizeid());
		uint64_t hashKey = desc.prizeid();
...
{
	return const_cast<proto_ff_s::arenarankaward_s *>((static_cast<const ArenaRankawardDesc*>(this))->GetDesc(id));
}
