...
int ArenaRankawardDesc::Load(NFResDB *pDB)
{
	...
	if ((table.arenarankaward_list_size() < 0) || (table.arenarankaward_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.arenarankaward_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.arenarankaward_list_size());
	...
	for (int i = 0; i < (int)table.arenarankaward_list_size(); i++)
	{
		...
		auto pIndex = m_astDescMap.Insert(desc.prizeid());
		CHECK_EXPR(pIndex, -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.prizeid());
		*pIndex = i;
