...
int MountEquippartDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_mountequippart_list_size() < 0) || (table.e_mountequippart_list_size() > (int)(m_astDesc.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_mountequippart_list_size(); i++)
	{
		...
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_part());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_part(), curIndex);
		CHECK_EXPR_ASSERT(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_part());
