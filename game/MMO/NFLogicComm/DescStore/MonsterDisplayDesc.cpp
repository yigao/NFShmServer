...
int MonsterDisplayDesc::Load(NFResDB *pDB)
{
	...
	if ((table.monsterdisplay_list_size() < 0) || (table.monsterdisplay_list_size() > (int)(m_astDesc.size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.monsterdisplay_list_size());
		return -2;
	}

	m_astDesc.resize(table.monsterdisplay_list_size());
	m_astDescIndex.resize(m_astDescIndex.max_size());
	for(int i = 0; i < (int)m_astDescIndex.size(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.monsterdisplay_list_size(); i++)
	{
		const proto_ff::monsterdisplay& desc = table.monsterdisplay_list(i);
		if (desc.has_displayid() == false && desc.ByteSize() == 0)
		{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.displayid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.displayid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.displayid());
		uint64_t hashKey = desc.displayid();
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.monsterdisplay_list_size());
...
const proto_ff_s::monsterdisplay_s * MonsterDisplayDesc::GetDesc(int id) const
{
...
{
	return const_cast<proto_ff_s::monsterdisplay_s *>((static_cast<const MonsterDisplayDesc*>(this))->GetDesc(id));
}
