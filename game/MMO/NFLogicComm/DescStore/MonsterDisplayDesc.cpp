...
int MonsterDisplayDesc::Load(NFResDB *pDB)
{
	...
	m_astDesc.resize(table.e_monsterdisplay_list_size());
	...
	for (int i = 0; i < (int)table.e_monsterdisplay_list_size(); i++)
	{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_displayid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_displayid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_displayid());
		uint64_t hashKey = desc.m_displayid();
