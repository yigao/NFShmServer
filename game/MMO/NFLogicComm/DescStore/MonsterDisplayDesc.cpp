...
int MonsterDisplayDesc::Load(NFResDB *pDB)
{
	...
	if ((table.monsterdisplay_list_size() < 0) || (table.monsterdisplay_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.monsterdisplay_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.monsterdisplay_list_size());
	...
	for (int i = 0; i < (int)table.monsterdisplay_list_size(); i++)
	{
		...
		auto pIndex = m_astDescMap.Insert(desc.displayid());
		CHECK_EXPR(pIndex, -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.displayid());
		*pIndex = i;
