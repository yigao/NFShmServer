...
IMPLEMENT_IDCREATE_WITHTYPE_GLOBAL(MonsterDisplayDesc, EOT_CONST_MONSTER_DISPLAY_DESC_ID, NFShmObj)

MonsterDisplayDesc::MonsterDisplayDesc():NFIDescStore()
{
...
int MonsterDisplayDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_monsterdisplay_list_size() < 0) || (table.e_monsterdisplay_list_size() > (int)(m_astDesc.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_monsterdisplay_list_size(); i++)
	{
		...
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_displayid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_displayid(), curIndex);
		CHECK_EXPR_ASSERT(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_displayid());
...
const proto_ff_s::E_MonsterDisplay_s * MonsterDisplayDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_MonsterDisplay_s * MonsterDisplayDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_MonsterDisplay_s *>((static_cast<const MonsterDisplayDesc*>(this))->GetDesc(id));
}

int MonsterDisplayDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_MonsterDisplay_s * MonsterDisplayDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_MonsterDisplay_s * MonsterDisplayDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
