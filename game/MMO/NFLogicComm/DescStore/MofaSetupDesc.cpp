...
IMPLEMENT_IDCREATE_WITHTYPE_GLOBAL(MofaSetupDesc, EOT_CONST_MOFA_SETUP_DESC_ID, NFShmObj)

MofaSetupDesc::MofaSetupDesc():NFIDescStore()
{
...
int MofaSetupDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_mofasetup_list_size() < 0) || (table.e_mofasetup_list_size() > (int)(m_astDesc.max_size())))
	{
...
const proto_ff_s::E_MofaSetup_s * MofaSetupDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_MofaSetup_s * MofaSetupDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_MofaSetup_s *>((static_cast<const MofaSetupDesc*>(this))->GetDesc(id));
}

int MofaSetupDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_MofaSetup_s * MofaSetupDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_MofaSetup_s * MofaSetupDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
