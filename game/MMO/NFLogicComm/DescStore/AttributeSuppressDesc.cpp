...
int AttributeSuppressDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_attributesuppress_list_size() < 0) || (table.e_attributesuppress_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_attributesuppress_list_size(); i++)
	{
		...
		{
			m_minId = desc.has_m_lvldiff();
		}
		else
		{
			if (desc.has_m_lvldiff() < m_minId)
			{
				m_minId = desc.has_m_lvldiff();
			}
		}
		...
		auto pDesc = &m_astDescMap[desc.m_lvldiff()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_lvldiff());
		pDesc->read_from_pbmsg(desc);
