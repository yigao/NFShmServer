...
AttributeSuppressDesc::AttributeSuppressDesc(NFIPluginManager* pPluginManager):NFIDescStore(pPluginManager)
{
...
int AttributeSuppressDesc::Load(NFResDB *pDB)
{
	...
	if ((table.attributesuppress_list_size() < 0) || (table.attributesuppress_list_size() > (int)(m_astDesc.GetSize())))
	{
		NFLogError(NF_LOG_COMM_LOGIC_PLUGIN, 0, "Invalid TotalNum:{}", table.attributesuppress_list_size());
		return -2;
	}

	for (int i = 0; i < table.attributesuppress_list_size(); i++)
	{
		const proto_ff::attributesuppress& desc = table.attributesuppress_list(i);
		auto pDesc = m_astDesc.Insert(desc.lvldiff());
		CHECK_EXPR(pDesc, -1, "m_astDesc.Insert Failed desc.id:{}", desc.lvldiff());
		pDesc->read_from_pbmsg(desc);
	}

	NFLogTrace(NF_LOG_COMM_LOGIC_PLUGIN, 0, "load {}, num={}", iRet, table.attributesuppress_list_size());
	return 0;
