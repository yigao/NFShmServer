...
int RoleAddskillDesc::Load(NFResDB *pDB)
{
	...
	if ((table.roleaddskill_list_size() < 0) || (table.roleaddskill_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.roleaddskill_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.roleaddskill_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.roleaddskill_list_size(); i++)
	{
		const proto_ff::roleaddSkill& desc = table.roleaddskill_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.roleaddskill_list_size());
...
proto_ff_s::roleaddSkill_s * RoleAddskillDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::roleaddSkill_s *>((static_cast<const RoleAddskillDesc*>(this))->GetDesc(id));
}
