...
int FlashFlashDesc::Load(NFResDB *pDB)
{
	...
	if ((table.flashflash_list_size() < 0) || (table.flashflash_list_size() > (int)(m_astDesc.GetMaxSize())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.flashflash_list_size());
		return -2;
	}

	m_astDesc.SetSize(table.flashflash_list_size());
	m_astDescIndex.SetSize(m_astDescIndex.GetMaxSize());
	for(int i = 0; i < (int)m_astDescIndex.GetSize(); i++)
	{
		m_astDescIndex[i] = -1;
	}
	for (int i = 0; i < (int)table.flashflash_list_size(); i++)
	{
		const proto_ff::flashflash& desc = table.flashflash_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.flashflash_list_size());
...
const proto_ff_s::flashflash_s * FlashFlashDesc::GetDesc(int id) const
{
...
proto_ff_s::flashflash_s * FlashFlashDesc::GetDesc(int id)
{
	return const_cast<proto_ff_s::flashflash_s *>((static_cast<const FlashFlashDesc*>(this))->GetDesc(id));
}
