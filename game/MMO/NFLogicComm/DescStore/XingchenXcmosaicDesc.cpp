...
IMPLEMENT_IDCREATE_WITHTYPE(XingchenXcmosaicDesc, EOT_CONST_XINGCHEN_XCMOSAIC_DESC_ID, NFShmObj)
...
int XingchenXcmosaicDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "XingchenXcmosaicDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_XingchenXcmosaic table;
	...
	if ((table.e_xingchenxcmosaic_list_size() < 0) || (table.e_xingchenxcmosaic_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_xingchenxcmosaic_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_xingchenxcmosaic_list_size(); i++)
	{
		const proto_ff::E_XingchenXcmosaic& desc = table.e_xingchenxcmosaic_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_xingchenxcmosaic_list_size());
