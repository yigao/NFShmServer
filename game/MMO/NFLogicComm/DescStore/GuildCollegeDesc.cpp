...
int GuildCollegeDesc::Load(NFResDB *pDB)
{
	...
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.guildcollege_list_size());
		return -2;
	}
	...
	for (int i = 0; i < table.guildcollege_list_size(); i++)
	{
		...
		if (desc.has_practiceid() == false && desc.ByteSize() == 0)
		{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.guildcollege_list_size());
...
const proto_ff_s::guildcollege_s * GuildCollegeDesc::GetDesc(int id) const
{
