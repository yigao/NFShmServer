#include "FestivalMuban_play_gameDesc.h"
#include "BoxBoxDesc.h"
#include "NFComm/NFPluginModule/NFCheck.h"

IMPLEMENT_IDCREATE_WITHTYPE(FestivalMuban_play_gameDesc, EOT_CONST_FESTIVAL_MUBAN_PLAY_GAME_DESC_ID, NFShmObj)
...
int FestivalMuban_play_gameDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalMuban_play_gameDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_FestivalMuban_play_game table;
	...
	if ((table.e_festivalmuban_play_game_list_size() < 0) || (table.e_festivalmuban_play_game_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivalmuban_play_game_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_festivalmuban_play_game_list_size(); i++)
	{
		const proto_ff::E_FestivalMuban_play_game& desc = table.e_festivalmuban_play_game_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_festivalmuban_play_game_list_size());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--end--");
	return 0;
}

int FestivalMuban_play_gameDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		auto pDesc = &m_astDesc[i];
		CHECK_EXPR_MSG_RESULT((pDesc->m_rewardbox <= 0 || BoxBoxDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_rewardbox)), result, "can't find the rewardbox:{} in the  excel:box sheet:box", pDesc->m_rewardbox);
	}
	return result;
}
