...
IMPLEMENT_IDCREATE_WITHTYPE_GLOBAL(RankingRankingDesc, EOT_CONST_RANKING_RANKING_DESC_ID, NFShmObj)

RankingRankingDesc::RankingRankingDesc():NFIDescStore()
{
...
const proto_ff_s::E_RankingRanking_s * RankingRankingDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_RankingRanking_s * RankingRankingDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_RankingRanking_s *>((static_cast<const RankingRankingDesc*>(this))->GetDesc(id));
}

int RankingRankingDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_RankingRanking_s * RankingRankingDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_RankingRanking_s * RankingRankingDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
