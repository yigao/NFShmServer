...
int RankingRankingDesc::Load(NFResDB *pDB)
{
	...
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.rankingranking_list_size());
		return -2;
	}
	...
	for (int i = 0; i < table.rankingranking_list_size(); i++)
	{
		...
		if (desc.has_rankingid() == false && desc.ByteSize() == 0)
		{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.rankingranking_list_size());
