#include "FestivalTemplateDesc.h"
...
FestivalTemplateDesc::~FestivalTemplateDesc()
{
}

int FestivalTemplateDesc::CreateInit()
{
	return Initialize();
}

int FestivalTemplateDesc::ResumeInit()
{
	return 0;
}

int FestivalTemplateDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "FestivalTemplateDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_FestivalTemplate table;
	...
	if ((table.e_festivaltemplate_list_size() < 0) || (table.e_festivaltemplate_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_festivaltemplate_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_festivaltemplate_list_size(); i++)
	{
		const proto_ff::E_FestivalTemplate& desc = table.e_festivaltemplate_list(i);
...
int FestivalTemplateDesc::CheckWhenAllDataLoaded()
{
	int result = 0;
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		auto pDesc = &m_astDesc[i];
		CHECK_EXPR_MSG_RESULT(FestivalFestivalDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_festivalid), result, "can't find the festivalid:{} in the  excel:festival sheet:festival", pDesc->m_festivalid);
	}
	return result;
}

const proto_ff_s::E_FestivalTemplate_s * FestivalTemplateDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_FestivalTemplate_s * FestivalTemplateDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_FestivalTemplate_s *>((static_cast<const FestivalTemplateDesc*>(this))->GetDesc(id));
}

int FestivalTemplateDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_FestivalTemplate_s * FestivalTemplateDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_FestivalTemplate_s * FestivalTemplateDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
