...
int ShenequipClearDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "ShenequipClearDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_ShenequipClear table;
	...
	if ((table.e_shenequipclear_list_size() < 0) || (table.e_shenequipclear_list_size() > (int)(m_astDescMap.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_shenequipclear_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_shenequipclear_list_size(); i++)
	{
		const proto_ff::E_ShenequipClear& desc = table.e_shenequipclear_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_shenequipclear_list_size());
