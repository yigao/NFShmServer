...
IMPLEMENT_IDCREATE_WITHTYPE(ShentiSuitDesc, EOT_CONST_SHENTI_SUIT_DESC_ID, NFShmObj)
...
int ShentiSuitDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "ShentiSuitDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_ShentiSuit table;
	...
	if ((table.e_shentisuit_list_size() < 0) || (table.e_shentisuit_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_shentisuit_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_shentisuit_list_size(); i++)
	{
		const proto_ff::E_ShentiSuit& desc = table.e_shentisuit_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_shentisuit_list_size());
