...
IMPLEMENT_IDCREATE_WITHTYPE(BattlepassShopDesc, EOT_CONST_BATTLEPASS_SHOP_DESC_ID, NFShmObj)
...
int BattlepassShopDesc::Load(NFResDB *pDB)
{
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "--begin--");
	CHECK_EXPR(pDB != NULL, -1, "pDB == NULL");

	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "BattlepassShopDesc::Load() strFileName = {}", GetFileName());

	proto_ff::Sheet_BattlepassShop table;
	...
	if ((table.e_battlepassshop_list_size() < 0) || (table.e_battlepassshop_list_size() > (int)(m_astDesc.max_size())))
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.e_battlepassshop_list_size());
		return -2;
	}

	for (int i = 0; i < (int)table.e_battlepassshop_list_size(); i++)
	{
		const proto_ff::E_BattlepassShop& desc = table.e_battlepassshop_list(i);
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.e_battlepassshop_list_size());
