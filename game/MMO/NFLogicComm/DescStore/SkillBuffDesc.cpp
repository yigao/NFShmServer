...
int SkillBuffDesc::Load(NFResDB *pDB)
{
	...
	if ((table.e_skillbuff_list_size() < 0) || (table.e_skillbuff_list_size() > (int)(m_astDescMap.max_size())))
	{
	...
	for (int i = 0; i < (int)table.e_skillbuff_list_size(); i++)
	{
		...
		if (desc.m_buffid() == 0 || desc.ByteSize() == 0)
		{
		...
		auto pDesc = &m_astDescMap[desc.m_buffid()];
		CHECK_EXPR_ASSERT(pDesc, -1, "m_astDescMap Insert Failed desc.id:{}", desc.m_buffid());
		pDesc->read_from_pbmsg(desc);
