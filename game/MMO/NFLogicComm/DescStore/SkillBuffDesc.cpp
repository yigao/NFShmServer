...
int SkillBuffDesc::Load(NFResDB *pDB)
{
	...
	m_astDesc.resize(table.e_skillbuff_list_size());
	...
	for (int i = 0; i < (int)table.e_skillbuff_list_size(); i++)
	{
		...
		CHECK_EXPR(pDesc, -1, "m_astDesc Index Failed desc.id:{}", desc.m_buffid());
		pDesc->read_from_pbmsg(desc);
		auto iter = m_astDescMap.emplace_hint(desc.m_buffid(), i);
		CHECK_EXPR(iter != m_astDescMap.end(), -1, "m_astDescMap.Insert Failed desc.id:{}, key maybe exist", desc.m_buffid());
		uint64_t hashKey = desc.m_buffid();
...
const proto_ff_s::E_SkillBuff_s * SkillBuffDesc::GetDesc(int64_t id) const
{
...
proto_ff_s::E_SkillBuff_s * SkillBuffDesc::GetDesc(int64_t id)
{
	return const_cast<proto_ff_s::E_SkillBuff_s *>((static_cast<const SkillBuffDesc*>(this))->GetDesc(id));
}

int SkillBuffDesc::GetDescIndex(int id) const
{
	auto iter = m_astDescMap.find(id);
	if (iter != m_astDescMap.end())
	{
...
const proto_ff_s::E_SkillBuff_s * SkillBuffDesc::GetDescByIndex(int index) const
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}

proto_ff_s::E_SkillBuff_s * SkillBuffDesc::GetDescByIndex(int index)
{
	CHECK_EXPR_ASSERT(index < (int)m_astDesc.size(), NULL, "the index:{} exist error, than the m_astDesc max index:{}", index, m_astDesc.size());
	return &m_astDesc[index];
}
