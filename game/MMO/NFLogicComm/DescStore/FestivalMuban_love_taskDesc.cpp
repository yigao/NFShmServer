...
int FestivalMuban_love_taskDesc::CheckWhenAllDataLoaded()
{
	...
	for(int i = 0; i < (int)m_astDesc.size(); i++)
	{
		...
		CHECK_EXPR_MSG_RESULT(BoxBoxDesc::Instance(m_pObjPluginManager)->GetDesc(pDesc->m_reward), result, "can't find the reward:{} in the  excel:box sheet:box", pDesc->m_reward);
	}
	return result;
}
