...
E_RoleChangecareerAttributeDesc_s::E_RoleChangecareerAttributeDesc_s() {
	if (EN_OBJ_MODE_INIT == NFShmMgr::Instance()->GetCreateMode()) {
		CreateInit();
	} else {
		ResumeInit();
...
int E_RoleChangecareerAttributeDesc_s::CreateInit() {
	m_value = (int32_t)0;
	m_type = (int32_t)0;
	return 0;
}

int E_RoleChangecareerAttributeDesc_s::ResumeInit() {
	return 0;
}

void E_RoleChangecareerAttributeDesc_s::write_to_pbmsg(::proto_ff::E_RoleChangecareerAttributeDesc & msg) const {
	msg.set_m_value((int32_t)m_value);
	msg.set_m_type((int32_t)m_type);
}

void E_RoleChangecareerAttributeDesc_s::read_from_pbmsg(const ::proto_ff::E_RoleChangecareerAttributeDesc & msg) {
	m_value = msg.m_value();
	m_type = msg.m_type();
}

E_RoleChangecareer_s::E_RoleChangecareer_s() {
	if (EN_OBJ_MODE_INIT == NFShmMgr::Instance()->GetCreateMode()) {
		CreateInit();
	} else {
		ResumeInit();
...
int E_RoleChangecareer_s::CreateInit() {
	m_professionID = (int32_t)0;
	m_rechargeID = (int32_t)0;
	return 0;
}

int E_RoleChangecareer_s::ResumeInit() {
	return 0;
}

void E_RoleChangecareer_s::write_to_pbmsg(::proto_ff::E_RoleChangecareer & msg) const {
	msg.set_m_professionid((int32_t)m_professionID);
	msg.set_m_rechargeid((int32_t)m_rechargeID);
	for(int32_t i = 0; i < (int32_t)m_attribute.size(); ++i) {
		::proto_ff::E_RoleChangecareerAttributeDesc* temp_m_attribute = msg.add_m_attribute();
		m_attribute[i].write_to_pbmsg(*temp_m_attribute);
	}
}

void E_RoleChangecareer_s::read_from_pbmsg(const ::proto_ff::E_RoleChangecareer & msg) {
	m_professionID = msg.m_professionid();
	m_rechargeID = msg.m_rechargeid();
	m_attribute.resize((int)msg.m_attribute_size() > (int)m_attribute.max_size() ? m_attribute.max_size() : msg.m_attribute_size());
	for(int32_t i = 0; i < (int32_t)m_attribute.size(); ++i) {
		const ::proto_ff::E_RoleChangecareerAttributeDesc & temp_m_attribute = msg.m_attribute(i);
		m_attribute[i].read_from_pbmsg(temp_m_attribute);
	}
}

Sheet_RoleChangecareer_s::Sheet_RoleChangecareer_s() {
	if (EN_OBJ_MODE_INIT == NFShmMgr::Instance()->GetCreateMode()) {
		CreateInit();
	} else {
		ResumeInit();
...
int Sheet_RoleChangecareer_s::CreateInit() {
	return 0;
}

int Sheet_RoleChangecareer_s::ResumeInit() {
	return 0;
}

void Sheet_RoleChangecareer_s::write_to_pbmsg(::proto_ff::Sheet_RoleChangecareer & msg) const {
	for(int32_t i = 0; i < (int32_t)E_RoleChangecareer_List.size(); ++i) {
		::proto_ff::E_RoleChangecareer* temp_e_rolechangecareer_list = msg.add_e_rolechangecareer_list();
		E_RoleChangecareer_List[i].write_to_pbmsg(*temp_e_rolechangecareer_list);
	}
}

void Sheet_RoleChangecareer_s::read_from_pbmsg(const ::proto_ff::Sheet_RoleChangecareer & msg) {
	E_RoleChangecareer_List.resize((int)msg.e_rolechangecareer_list_size() > (int)E_RoleChangecareer_List.max_size() ? E_RoleChangecareer_List.max_size() : msg.e_rolechangecareer_list_size());
	for(int32_t i = 0; i < (int32_t)E_RoleChangecareer_List.size(); ++i) {
		const ::proto_ff::E_RoleChangecareer & temp_e_rolechangecareer_list = msg.e_rolechangecareer_list(i);
		E_RoleChangecareer_List[i].read_from_pbmsg(temp_e_rolechangecareer_list);
	}
}

E_RoleCareerpreposeRewardDesc_s::E_RoleCareerpreposeRewardDesc_s() {
	if (EN_OBJ_MODE_INIT == NFShmMgr::Instance()->GetCreateMode()) {
		CreateInit();
	} else {
		ResumeInit();
...
int E_RoleCareerpreposeRewardDesc_s::CreateInit() {
	m_num = (int32_t)0;
	m_item = (int32_t)0;
	return 0;
}

int E_RoleCareerpreposeRewardDesc_s::ResumeInit() {
	return 0;
}

void E_RoleCareerpreposeRewardDesc_s::write_to_pbmsg(::proto_ff::E_RoleCareerpreposeRewardDesc & msg) const {
	msg.set_m_num((int32_t)m_num);
	msg.set_m_item((int32_t)m_item);
}

void E_RoleCareerpreposeRewardDesc_s::read_from_pbmsg(const ::proto_ff::E_RoleCareerpreposeRewardDesc & msg) {
	m_num = msg.m_num();
	m_item = msg.m_item();
}

E_RoleCareerprepose_s::E_RoleCareerprepose_s() {
	if (EN_OBJ_MODE_INIT == NFShmMgr::Instance()->GetCreateMode()) {
		CreateInit();
	} else {
		ResumeInit();
...
int E_RoleCareerprepose_s::CreateInit() {
	m_id = (int32_t)0;
...
int E_RoleCareerprepose_s::ResumeInit() {
	return 0;
}

void E_RoleCareerprepose_s::write_to_pbmsg(::proto_ff::E_RoleCareerprepose & msg) const {
	msg.set_m_id((int32_t)m_id);
	msg.set_m_condition((int32_t)m_condition);
	msg.set_m_value((int32_t)m_value);
	for(int32_t i = 0; i < (int32_t)m_reward.size(); ++i) {
		::proto_ff::E_RoleCareerpreposeRewardDesc* temp_m_reward = msg.add_m_reward();
		m_reward[i].write_to_pbmsg(*temp_m_reward);
	}
}

void E_RoleCareerprepose_s::read_from_pbmsg(const ::proto_ff::E_RoleCareerprepose & msg) {
	m_id = msg.m_id();
	...
	for(int32_t i = 0; i < (int32_t)m_reward.size(); ++i) {
		const ::proto_ff::E_RoleCareerpreposeRewardDesc & temp_m_reward = msg.m_reward(i);
		m_reward[i].read_from_pbmsg(temp_m_reward);
	}
}

Sheet_RoleCareerprepose_s::Sheet_RoleCareerprepose_s() {
	if (EN_OBJ_MODE_INIT == NFShmMgr::Instance()->GetCreateMode()) {
		CreateInit();
	} else {
		ResumeInit();
...
int Sheet_RoleCareerprepose_s::CreateInit() {
	return 0;
}

int Sheet_RoleCareerprepose_s::ResumeInit() {
	return 0;
}

void Sheet_RoleCareerprepose_s::write_to_pbmsg(::proto_ff::Sheet_RoleCareerprepose & msg) const {
	for(int32_t i = 0; i < (int32_t)E_RoleCareerprepose_List.size(); ++i) {
		::proto_ff::E_RoleCareerprepose* temp_e_rolecareerprepose_list = msg.add_e_rolecareerprepose_list();
		E_RoleCareerprepose_List[i].write_to_pbmsg(*temp_e_rolecareerprepose_list);
	}
}

void Sheet_RoleCareerprepose_s::read_from_pbmsg(const ::proto_ff::Sheet_RoleCareerprepose & msg) {
	E_RoleCareerprepose_List.resize((int)msg.e_rolecareerprepose_list_size() > (int)E_RoleCareerprepose_List.max_size() ? E_RoleCareerprepose_List.max_size() : msg.e_rolecareerprepose_list_size());
	for(int32_t i = 0; i < (int32_t)E_RoleCareerprepose_List.size(); ++i) {
		const ::proto_ff::E_RoleCareerprepose & temp_e_rolecareerprepose_list = msg.e_rolecareerprepose_list(i);
		E_RoleCareerprepose_List[i].read_from_pbmsg(temp_e_rolecareerprepose_list);
	}
}
