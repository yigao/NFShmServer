...
void E_TitleTitleAttributeDesc_s::read_from_pbmsg(const ::proto_ff::E_TitleTitleAttributeDesc & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_TitleTitleAttributeDesc_s));
	m_type = msg.m_type();
	m_value = msg.m_value();
}
...
void E_TitleTitle_s::write_to_pbmsg(::proto_ff::E_TitleTitle & msg) const {
	...
	msg.set_m_eventvalue((const char*)m_eventvalue.data());
	msg.set_m_describe((const char*)m_describe.data());
...
void E_TitleTitle_s::read_from_pbmsg(const ::proto_ff::E_TitleTitle & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_TitleTitle_s));
	m_id = msg.m_id();
	m_name = msg.m_name();
	m_type = msg.m_type();
