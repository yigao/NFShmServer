...
{
	if ((int32_t)ETimeType::permanent == tplcfg->timetype) return (uint8_t)EFestState::fest_open;
	if ((int32_t)ETimeType::openserver == tplcfg->timetype)
	{
		if ((int64_t)EServType::newzone == tplcfg->timeparam3)
		{
			if (!param.isret || param.ismerge) return (uint8_t)EFestState::fest_not_open;
		}
		else if ((int64_t)EServType::merge == tplcfg->timeparam3)
		{
			if (!param.isret || !param.ismerge) return (uint8_t)EFestState::fest_not_open;
		}
		if (param.days < tplcfg->timeparam1) return (uint8_t)EFestState::fest_not_open;
		if (param.days > tplcfg->timeparam2) return (uint8_t)EFestState::fest_end;
		return (uint8_t)EFestState::fest_open;
	}
	if ((int32_t)ETimeType::date == tplcfg->timetype)
	{
		if (param.curtime < (uint64_t)tplcfg->timeparam1) return (uint8_t)EFestState::fest_not_open;
		if (tplcfg->timeparam3 > 0)
		{
			uint64_t begin = param.zerotime + tplcfg->timeparam3 * 24 * 3600;
			if (param.curtime < begin) return (uint8_t)EFestState::fest_not_open;
		}
		if (param.curtime >= (uint64_t)tplcfg->timeparam2) return (uint8_t)EFestState::fest_end;
		return (uint8_t)EFestState::fest_open;
	}
	return (uint8_t)EFestState::fest_not_open;
}
