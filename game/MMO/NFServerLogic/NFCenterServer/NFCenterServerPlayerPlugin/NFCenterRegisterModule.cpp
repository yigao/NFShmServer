...
int NFCenterRegisterModule::OnRpcServiceCrossMapReg(proto_ff::RegisterCrossMapInfoReq& request, proto_ff::ReigsterCrossMapInfoRsp& respone)
{
	auto pConfig = FindModule<NFIConfigModule>()->GetAppConfig(NF_ST_CENTER_SERVER);
	CHECK_NULL(pConfig);
	CHECK_EXPR(pConfig->IsCrossServer(), -1, "");

	for (int i = 0; i < (int)request.map_size(); i++)
	{
		auto& mapAddr = request.map(i);
		uint64_t mapId = mapAddr.map_id();
		uint32_t busId = mapAddr.bus_id();
		uint32_t zid = NFServerIDUtil::GetZoneID(busId);
		bool crossServer = mapAddr.cross_server();
		NFServerAddrMgr::Instance(m_pObjPluginManager)->AddMapAddr(mapId, busId, zid, crossServer);
		NFLogInfo(NF_LOG_SYSTEMLOG, 0, "Center Server:{} Register GameServer:{} Map:{} To Cross Center Server:{}({})", zid, NFServerIDUtil::GetBusNameFromBusID(busId), mapId, pConfig->ServerName, pConfig->ServerId);
	}
	return 0;
}
