...
int NFPlayer::NotifyPlayerInfo()
{
    ...
    rsp.set_zid(GetZid());
    //rsp.set_opentime(g_GetLogicService()->GetOpenTime(GetZid()));
...
/*    rsp.set_world_lv(g_GetGlobalMgr()->GetWorldLv(m_zid));
    ...
    rsp.set_world_lv(0);
    rsp.set_world_lv_4(0);
    rsp.set_world_lv_8(0);
    rsp.set_world_lv_16(0);
    rsp.set_is_merge(false);
    //
...
bool NFPlayer::AddAttrCache(uint32_t ANum, int64_t attrValue, SCommonSource *pSource /*= nullptr*/)
{
    ...
    else if(proto_ff::A_GOD_META_EXP == ANum){//神元经验
