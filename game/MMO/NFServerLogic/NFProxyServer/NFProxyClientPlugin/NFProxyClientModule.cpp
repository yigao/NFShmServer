...
bool NFCProxyClientModule::Init()
{
...

    {
        ...
        NFShmHashMap<int, NFSizeString<32>, 1000000, 500000> hashMap;
        for(int i = 0; i < 1000000; i++)
        {
            NFSizeString<32> tmp;
            tmp = NFCommon::tostr(vec[i]);
            int ret = hashMap.Insert(vec[i], tmp);
            if (ret != 0)
            {
                NFLogError(NF_LOG_SYSTEMLOG, 0, "hahsMap Insert failed, i:{} ret:{}", vec[i], ret);
            }
        }
        ...
        for(int i = 0; i < 1000000; i++)
        {
            NFSizeString<32>* pTmp = hashMap.Find(vec[i]);
            if (pTmp == NULL || pTmp->GetString() != NFCommon::tostr(vec[i]))
            {
    ...
    {
        uint64_t startTime = NFGetTime();
        NFShmHashMap<int, NFSizeString<32>, 1000000, 200000> hashMap;
        for(int i = 0; i < 1000000; i++)
        {
            NFSizeString<32> tmp;
            tmp = NFCommon::tostr(vec[i]);
    ...
    {
        uint64_t startTime = NFGetTime();
        NFShmHashMap<int, NFSizeString<32>, 1000000, 50000> hashMap;
        for(int i = 0; i < 1000000; i++)
        {
            NFSizeString<32> tmp;
            tmp = NFCommon::tostr(vec[i]);
    ...
    {
        uint64_t startTime = NFGetTime();
        std::map<int, NFSizeString<32>> hashMap;
        for(int i = 0; i < 1000000; i++)
        {
            NFSizeString<32> tmp;
            tmp = NFCommon::tostr(vec[i]);
            hashMap.emplace(vec[i], tmp);
        }
        ...
        for(int i = 0; i < 1000000; i++)
        {
            ...
            if (iter != hashMap.end())
            {
                NFSizeString<32>* pTmp = &(iter->second);
                if (pTmp->GetString() != NFCommon::tostr(vec[i]))
                {
                    NFLogError(NF_LOG_SYSTEMLOG, 0, "hahsMap Find failed, i:{} ", vec[i]);
                }
            }
    ...
    {
        uint64_t startTime = NFGetTime();
        std::unordered_map<int, NFSizeString<32>> hashMap;
        for(int i = 0; i < 1000000; i++)
        {
            NFSizeString<32> tmp;
            tmp = NFCommon::tostr(vec[i]);
