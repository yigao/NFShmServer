...
int MountMountDesc::Load(NFResDB *pDB)
{
	...
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.mountmount_list_size());
		return -2;
	}
	...
	for (int i = 0; i < table.mountmount_list_size(); i++)
	{
		...
		if (desc.has_mountid() == false && desc.ByteSize() == 0)
		{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.mountmount_list_size());
...
const proto_ff_s::mountmount_s * MountMountDesc::GetDesc(int id) const
{
