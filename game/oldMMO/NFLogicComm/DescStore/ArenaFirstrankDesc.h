#pragma once

#include "NFServerComm/NFDescStorePlugin/NFIDescStore.h"
#include "NFComm/NFShmCore/NFShmMgr.h"
#include "NFLogicCommon/NFDescStoreTypeDefines.h"
#include "NFServerLogicMessage/arena_s.h"

#define MAX_ARENA_FIRSTRANK_NUM 20

class ArenaFirstrankDesc : public NFIDescStore
{
public:
	ArenaFirstrankDesc(NFIPluginManager* pPluginManager);
	virtual ~ArenaFirstrankDesc();
	int CreateInit();
	int ResumeInit();
	const proto_ff_s::arenafirstrank_s* GetDesc(int id) const;
	proto_ff_s::arenafirstrank_s* GetDesc(int id);
	const NFShmHashMap<uint64_t, proto_ff_s::arenafirstrank_s, MAX_ARENA_FIRSTRANK_NUM>* GetAllDesc() const { return &m_astDesc; }
	NFShmHashMap<uint64_t, proto_ff_s::arenafirstrank_s, MAX_ARENA_FIRSTRANK_NUM>* GetAllDesc() { return &m_astDesc; }
public:
IMPL_RES_HASH_DESC(proto_ff_s::arenafirstrank_s, arenafirstrank, MAX_ARENA_FIRSTRANK_NUM);
DECLARE_IDCREATE(ArenaFirstrankDesc);
};
