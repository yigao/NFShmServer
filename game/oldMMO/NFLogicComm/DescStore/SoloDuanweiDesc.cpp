...
int SoloDuanweiDesc::Load(NFResDB *pDB)
{
	...
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "Invalid TotalNum:{}", table.soloduanwei_list_size());
		return -2;
	}
	...
	for (int i = 0; i < table.soloduanwei_list_size(); i++)
	{
		...
		if (desc.has_duanweiid() == false && desc.ByteSize() == 0)
		{
	...
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "load {}, num={}", iRet, table.soloduanwei_list_size());
...
const proto_ff_s::soloduanwei_s * SoloDuanweiDesc::GetDesc(int id) const
{
