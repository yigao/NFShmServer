...
{
    m_packetMsgConfig.resize(proto_ff::CLIENT_MAX_CMD_ID_VALUE);
}
...
int NFPackageConfig::LoadConfig(const string &path, const string &server)
{
    ...
    for (int i = 0; i < (int) serverMsg.packetmsg_size(); ++i)
    {
        ...
        CHECK_EXPR_ASSERT((msg.cmd() > 0 && msg.cmd() < proto_ff::CLIENT_MAX_CMD_ID_VALUE), -1, "invalid msg:{}", msg.cmd());
...
{
    CHECK_EXPR((iMsgID > 0 && iMsgID < proto_ff::CLIENT_MAX_CMD_ID_VALUE), NULL, "invalid msg:{}", iMsgID);
