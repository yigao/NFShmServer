...
void E_TaskTask_s::write_to_pbmsg(::proto_ff::E_TaskTask & msg) const {
	...
	msg.set_m_showtype((const char*)m_showtype.data());
	msg.set_m_weights((int32_t)m_weights);
	msg.set_m_name((const char*)m_name.data());
	msg.set_m_rececond((const char*)m_rececond.data());
	msg.set_m_receadd((const char*)m_receadd.data());
	msg.set_m_comcond((const char*)m_comcond.data());
	msg.set_m_nexttask((int32_t)m_nexttask);
	msg.set_m_tracker((const char*)m_tracker.data());
	msg.set_m_subtype((const char*)m_subtype.data());
	msg.set_m_subnpc((int32_t)m_subnpc);
	msg.set_m_subdesc((const char*)m_subdesc.data());
	msg.set_m_subaward((int32_t)m_subaward);
	for(int32_t i = 0; i < (int32_t)m_submsg.size(); ++i) {
		msg.add_m_submsg((const char*)m_submsg[i].data());
	}
}

void E_TaskTask_s::read_from_pbmsg(const ::proto_ff::E_TaskTask & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_TaskTask_s));
	m_id = msg.m_id();
	m_minlv = msg.m_minlv();
	...
	m_subdesc = msg.m_subdesc();
	m_subaward = msg.m_subaward();
	m_submsg.resize((int)msg.m_submsg_size() > (int)m_submsg.max_size() ? m_submsg.max_size() : msg.m_submsg_size());
	for(int32_t i = 0; i < (int32_t)m_submsg.size(); ++i) {
		m_submsg[i] = msg.m_submsg(i);
	}
}
