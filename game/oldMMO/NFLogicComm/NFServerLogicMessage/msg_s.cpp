...
void E_MsgPhpmsg_s::write_to_pbmsg(::proto_ff::E_MsgPhpmsg & msg) const {
	...
	msg.set_m_content((const char*)m_content.data());
	msg.set_m_note((const char*)m_note.data());
}

void E_MsgPhpmsg_s::read_from_pbmsg(const ::proto_ff::E_MsgPhpmsg & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_MsgPhpmsg_s));
	m_id = msg.m_id();
