...
void E_ShenequipEquip_s::write_to_pbmsg(::proto_ff::E_ShenequipEquip & msg) const {
	...
	msg.set_m_decompose((const char*)m_decompose.data());
	...
	msg.set_m_secretskill((const char*)m_secretskill.data());
	msg.set_m_crypticskill((const char*)m_crypticskill.data());
	msg.set_m_passiveskill((const char*)m_passiveskill.data());
}

void E_ShenequipEquip_s::read_from_pbmsg(const ::proto_ff::E_ShenequipEquip & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipEquip_s));
	m_id = msg.m_id();
	m_name = msg.m_name();
	m_type = msg.m_type();
	m_grade = msg.m_grade();
...
void Sheet_ShenequipEquip_s::read_from_pbmsg(const ::proto_ff::Sheet_ShenequipEquip & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct Sheet_ShenequipEquip_s));
...
void E_ShenequipAttAttributeDesc_s::read_from_pbmsg(const ::proto_ff::E_ShenequipAttAttributeDesc & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipAttAttributeDesc_s));
	m_type = msg.m_type();
	m_value = msg.m_value();
}
...
void E_ShenequipAttStarDesc_s::read_from_pbmsg(const ::proto_ff::E_ShenequipAttStarDesc & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipAttStarDesc_s));
	m_type = msg.m_type();
	m_value = msg.m_value();
}
...
void E_ShenequipAtt_s::read_from_pbmsg(const ::proto_ff::E_ShenequipAtt & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipAtt_s));
	m_id = msg.m_id();
...
void Sheet_ShenequipAtt_s::read_from_pbmsg(const ::proto_ff::Sheet_ShenequipAtt & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct Sheet_ShenequipAtt_s));
...
void E_ShenequipLvup_s::read_from_pbmsg(const ::proto_ff::E_ShenequipLvup & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipLvup_s));
	m_lv = msg.m_lv();
...
void Sheet_ShenequipLvup_s::read_from_pbmsg(const ::proto_ff::Sheet_ShenequipLvup & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct Sheet_ShenequipLvup_s));
...
void E_ShenequipClear_s::read_from_pbmsg(const ::proto_ff::E_ShenequipClear & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipClear_s));
	m_id = msg.m_id();
	m_type = msg.m_type();
	m_lv = msg.m_lv();
	m_itemid = msg.m_itemid();
...
void Sheet_ShenequipClear_s::read_from_pbmsg(const ::proto_ff::Sheet_ShenequipClear & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct Sheet_ShenequipClear_s));
...
void E_ShenequipAwakenAttributeDesc_s::read_from_pbmsg(const ::proto_ff::E_ShenequipAwakenAttributeDesc & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipAwakenAttributeDesc_s));
	m_type = msg.m_type();
	m_value = msg.m_value();
}
...
void E_ShenequipAwaken_s::read_from_pbmsg(const ::proto_ff::E_ShenequipAwaken & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipAwaken_s));
	m_id = msg.m_id();
	m_type = msg.m_type();
	m_lv = msg.m_lv();
	m_itemid = msg.m_itemid();
...
void Sheet_ShenequipAwaken_s::read_from_pbmsg(const ::proto_ff::Sheet_ShenequipAwaken & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct Sheet_ShenequipAwaken_s));
...
void E_ShenequipStarupMaterialDesc_s::read_from_pbmsg(const ::proto_ff::E_ShenequipStarupMaterialDesc & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipStarupMaterialDesc_s));
	m_grade = msg.m_grade();
	m_quality = msg.m_quality();
	m_num = msg.m_num();
	m_type = msg.m_type();
...
void E_ShenequipStarup_s::read_from_pbmsg(const ::proto_ff::E_ShenequipStarup & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipStarup_s));
	m_id = msg.m_id();
	m_grade = msg.m_grade();
	m_quality = msg.m_quality();
	m_stars = msg.m_stars();
...
void Sheet_ShenequipStarup_s::read_from_pbmsg(const ::proto_ff::Sheet_ShenequipStarup & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct Sheet_ShenequipStarup_s));
...
void E_ShenequipPreposeRewardDesc_s::read_from_pbmsg(const ::proto_ff::E_ShenequipPreposeRewardDesc & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipPreposeRewardDesc_s));
	m_item = msg.m_item();
	m_num = msg.m_num();
}
...
void E_ShenequipPrepose_s::read_from_pbmsg(const ::proto_ff::E_ShenequipPrepose & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct E_ShenequipPrepose_s));
	m_id = msg.m_id();
...
void Sheet_ShenequipPrepose_s::read_from_pbmsg(const ::proto_ff::Sheet_ShenequipPrepose & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct Sheet_ShenequipPrepose_s));
