...
int areapath_s::CreateInit() {
	id = (int64_t)0;
	belongToSceneID = (int64_t)0;
	return 0;
}

int areapath_s::ResumeInit() {
	return 0;
}

void areapath_s::write_to_pbmsg(::proto_ff::areapath & msg) const {
	msg.set_id((int64_t)id);
	msg.set_belongtosceneid((int64_t)belongToSceneID);
}

void areapath_s::read_from_pbmsg(const ::proto_ff::areapath & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct areapath_s));
	id = msg.id();
	belongToSceneID = msg.belongtosceneid();
}

Sheet_areapath_s::Sheet_areapath_s() {
	if (EN_OBJ_MODE_INIT == NFShmMgr::Instance()->GetCreateMode()) {
		CreateInit();
	} else {
		ResumeInit();
...
int Sheet_areapath_s::CreateInit() {
	return 0;
}

int Sheet_areapath_s::ResumeInit() {
	return 0;
}

void Sheet_areapath_s::write_to_pbmsg(::proto_ff::Sheet_areapath & msg) const {
	for(int32_t i = 0; i < (int32_t)areapath_List.GetSize() && i < areapath_List.GetMaxSize(); ++i) {
		::proto_ff::areapath* temp_areapath_list = msg.add_areapath_list();
		areapath_List[i].write_to_pbmsg(*temp_areapath_list);
	}
}

void Sheet_areapath_s::read_from_pbmsg(const ::proto_ff::Sheet_areapath & msg) {
	//dont't use memset, the class maybe has virtual //memset(this, 0, sizeof(struct Sheet_areapath_s));
	areapath_List.SetSize(msg.areapath_list_size() > areapath_List.GetMaxSize() ? areapath_List.GetMaxSize() : msg.areapath_list_size());
	for(int32_t i = 0; i < (int32_t)areapath_List.GetSize(); ++i) {
		const ::proto_ff::areapath & temp_areapath_list = msg.areapath_list(i);
		areapath_List[i].read_from_pbmsg(temp_areapath_list);
	}
}
