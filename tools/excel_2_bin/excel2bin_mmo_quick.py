...
def fill_record(record, sheet, row_index, col_index, excel_sheet_col_count, recursion_count, parent_layertype, parent_layer_cname, is_start):
	""""""
	global g_sheet_object_field_proterties
	global g_layername_refcount
	global g_proterties_list 
	global g_step
	global g_now_step
	...
	index = g_layertype_instancecount[current_layer_typename]
	...
	while col_index < excel_sheet_col_count:
		...
		if is_start:
			...
			for field_property in g_sheet_object_field_proterties:
				...
				#print "[%s] [%s]" % (field_property_cname, current_layer_cname)
			...
			#print "[now step: %d]" % (g_now_step)
			if g_step[g_now_step] == True:
				field_property = g_proterties_list[g_now_step]
				field_property_cname = field_property[0]
				field_property_index = field_property[1]
				field_property_name  = field_property[2]
				field_property_label = field_property[3]
		...
		#message类型的repeated
			...
			col_index = fill_record(repeated_unit_msgrepeatedfield, sheet, row_index, col_index, \
									excel_sheet_col_count, recursion_count+1, current_layerid, input_layer_cname, is_start)
			
			#print "[size: %s]" % repeated_msgrepeatedfield
	...
	if -1 == last_ok_col_index and recursion_count > 0:
