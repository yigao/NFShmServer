...
public:
    ...
    *       所有的Fire事件最多迭代20次
    */
    virtual int FireExecute(uint32_t nEventID, uint64_t nSrcID, uint32_t bySrcType, const google::protobuf::Message &message) = 0;
    ...
    * @return			订阅事件是否成功
    */
    virtual int Subscribe(NFShmObj *pObj, uint32_t nEventID, uint64_t nSrcID, uint32_t bySrcType, const std::string &desc) = 0;
    ...
    * @return			取消订阅事件是否成功
    */
    virtual int UnSubscribe(NFShmObj *pObj, uint32_t nEventID, uint64_t nSrcID, uint32_t bySrcType) = 0;
