...
bool NFCWorldPlayerModule::Awake() {
    ...
    FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_PTW_PLAYER_LOGIN_MSG, this,
                                                       &NFCWorldPlayerModule::OnHandlePlayerLogin);
	FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_LTW_PLAYER_LOGIN_RSP, this,
                                                       &NFCWorldPlayerModule::OnHandleLogicServerLoginRsp);
    FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_PTW_PLAYER_RECONNECT_MSG, this,
                                                       &NFCWorldPlayerModule::OnHandlePlayerReconnect);
    FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_LTW_PLAYER_RECONNECT_MSG_RSP, this,
                                                       &NFCWorldPlayerModule::OnHandleLogicServerReconnectRsp);

    FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_PTW_PLAYER_DISCONNECT_MSG, this,
                                                       &NFCWorldPlayerModule::OnHandlePlayerDisConnect);

	FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_WORLD_ROUTER_CMD_TO_SUIT_LOGIC, this,
                                                    &NFCWorldPlayerModule::OnHandleRouteToSuitLogicServer);

    FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_LTW_REGISTER_USER_TO_WORLD_REQ, this,
                                                       &NFCWorldPlayerModule::OnHandleRegisterUserFromLoginServer);

    FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_LTW_PLAYER_LOGOUT_REQ, this,
                                                 &NFCWorldPlayerModule::OnHandleLogoutUserFromLoginServer);

    FindModule<NFIMessageModule>()->AddMessageCallBack(NF_ST_WORLD_SERVER, proto_ff::NF_LTL_PLAYER_DEVICE_CHANGE_NOTIFY, this,
...
int NFCWorldPlayerModule::OnHandlePlayerLogin(uint64_t unLinkId, uint64_t value, uint64_t proxyBusId, uint32_t nMsgId, const char *msg,
    ...
    if (pPlayerInfo == nullptr) {
        if (NFWorldPlayer::GetFreeCount() > 0)
        {
            ...
            if (pPlayerInfo == NULL)
            {
                ...
                FindModule<NFIMessageModule>()->SendMsgToProxyServer(NF_ST_WORLD_SERVER, xMsg.proxy_bus_id(),
	...
	FindModule<NFIMessageModule>()->SendMsgToLogicServer(NF_ST_WORLD_SERVER, pPlayerInfo->GetLogicId(), proto_ff::NF_WTL_PLAYER_LOGIN_REQ, rspLogicMsg);
...
int NFCWorldPlayerModule::OnHandleLogicServerReconnectRsp(uint64_t unLinkId, uint64_t value, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    ...
    FindModule<NFIMessageModule>()->SendMsgToProxyServer(NF_ST_WORLD_SERVER, pPlayerInfo->GetProxyId(),
...
int NFCWorldPlayerModule::OnHandlePlayerReconnect(uint64_t unLinkId, uint64_t value, uint64_t proxyBusId, uint32_t nMsgId, const char *msg,
    ...
    FindModule<NFIMessageModule>()->SendMsgToLogicServer(NF_ST_WORLD_SERVER, pPlayerInfo->GetLogicId(), proto_ff::NF_WTL_PLAYER_RECONNECT_MSG,
                                                    rspMsg);
    NFLogTrace(NF_LOG_WORLD_SERVER_PLUGIN, 0, "-- end --");
    return 0;
}
...
int NFCWorldPlayerModule::OnHandlePlayerDisConnect(uint64_t unLinkId, uint64_t value, uint64_t value2, uint32_t nMsgId, const char *msg,
    ...
    if (pPlayerInfo->GetLogicId() > 0)
    {
        FindModule<NFIMessageModule>()->SendMsgToLogicServer(NF_ST_WORLD_SERVER, pPlayerInfo->GetLogicId(), proto_ff::NF_WTL_PLAYER_DISCONNECT_MSG, xMsg);
    }
...
int NFCWorldPlayerModule::OnHandleRouteToSuitLogicServer(uint64_t unLinkId, uint64_t value, uint64_t value2, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    ...
    if (pPlayerInfo->GetLogicId() <= 0)
    {
        NF_SHARE_PTR<NFServerData> pLogicData = FindModule<NFIMessageModule>()->GetFirstServerByServerType(NF_ST_WORLD_SERVER, NF_ST_LOGIC_SERVER);
        if (pLogicData)
        {
            pPlayerInfo->SetLogicId(pLogicData->mServerInfo.bus_id());
        }
    ...
    if (pPlayerInfo->GetLogicId() > 0)
    {
        FindModule<NFIMessageModule>()->SendMsgToLogicServer(NF_ST_WORLD_SERVER, pPlayerInfo->GetLogicId(), xMsg.msg_id(), xMsg.msg_data().data(), xMsg.msg_data().length());
    }
    else
    {
...
int NFCWorldPlayerModule::OnHnadleChangeDeviceFromLoginServer(uint64_t unLinkId, uint64_t loginLinkId, uint64_t reqTransId, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    ...
    FindModule<NFIMessageModule>()->SendMsgToLogicServer(NF_ST_WORLD_SERVER, pPlayerInfo->GetLogicId(), nMsgId, xMsg);
    NFLogTrace(NF_LOG_WORLD_SERVER_PLUGIN, 0, "-- end --");
    return 0;
}
...
int NFCWorldPlayerModule::OnHandleRegisterUserFromLoginServer(uint64_t unLinkId, uint64_t loginLinkId, uint64_t reqTransId, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    ...
    if (pPlayerInfo == NULL)
    {
        ...
        FindModule<NFIMessageModule>()->SendTransToLoginServer(NF_ST_WORLD_SERVER, loginBusId, proto_ff::NF_LTL_REGISTER_USER_TO_LOGIN_RSP, rspMsg, 0, reqTransId);
    }
    ...
    FindModule<NFIMessageModule>()->SendMsgToLogicServer(NF_ST_WORLD_SERVER, pPlayerInfo->GetLogicId(), proto_ff::NF_WTL_REGISTER_USER_TO_LOGIC_REQ, reqMsg);
    NFLogTrace(NF_LOG_WORLD_SERVER_PLUGIN, 0, "-- end --");
    return 0;
}
