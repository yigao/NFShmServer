...
public:
	...
	*/
	 ...
	 * @return int			返回0错误
	 */
	virtual int64_t BindServer(NF_SERVER_TYPES eServerType, const std::string& url, uint32_t nNetThreadNum = 1, uint32_t nMaxConnectNum = 100, uint32_t nPacketParseType = PACKET_PARSE_TYPE_INTERNAL, bool bSecurity = false) override;
	...
	*/
    ...
    virtual void SendServer(uint64_t usLinkId, uint32_t nModuleId, uint32_t nMsgID, const char* msg, uint32_t nLen, uint64_t param1, uint64_t param2 = 0, uint64_t srcId = 0, uint64_t dstId = 0) override;
...
protected:
	 ...
	 *
	 */
    void SendMsg(NFINetMessage* pServer, uint64_t usLinkId, NFDataPackage& packet);
private:
	/**
	 * @brief	处理接受数据的回调
	 */
