...
typedef enum {
    ...
    EN_NFBUS_ERR_CHANNEL_UNSUPPORTED_VERSION = -106,   // 通道版本不受支持
    EN_NFBUS_ERR_CHANNEL_ALIGN_SIZE_MISMATCH = -107,   // 对齐参数不一致
    EN_NFBUS_ERR_CHANNEL_ARCH_SIZE_T_MISMATCH = -108,  // 架构size_t不匹配
    ...
    EN_NFBUS_ERR_SHM_CLOSE_FAILED = -303,  // 移除共享内存出错，具体错误原因可以查看errno或类似的位置
    EN_NFBUS_ERR_SHM_PATH_INVALID = -304,  // 共享内存地址错误
    EN_NFBUS_ERR_SHM_MAP_FAILED = -305,    // 共享内存地址映射错误
