...
void WriteMakeFile(std::string& excelmmo_gen, std::string& resmetas_gen, const std::string& file)
{
    ...
    resmetas_gen += "${PROTOCGEN_FILE_PATH}/" + file+"_proto_finish:${PROTOCOL_COMM_XML} ${FIELD_OPTIONS_XML} ${RESDB_META_PATH}/E_" + NFStringUtility::Capitalize(file) + ".proto\n";
    ...
    resmetas_gen += "\t${NFEXCELPROCESS} --work=\"exceltostruct\" --proto_ds=${PROTOCGEN_FILE_PATH}/" + file + ".proto.ds --src=E_" + NFStringUtility::Capitalize(file) + ".proto --dst=${PROTOCGEN_FILE_PATH}/;\n";
    resmetas_gen += "\t${FILE_COPY_EXE} --work=\"filecopy\" --src=\"${PROTOCGEN_FILE_PATH}/E_" + NFStringUtility::Capitalize(file) + ".pb.h ${PROTOCGEN_FILE_PATH}/E_" + NFStringUtility::Capitalize(file) + ".pb.cc ${PROTOCGEN_FILE_PATH}/E_" + NFStringUtility::Capitalize(file) + "_s.h ${PROTOCGEN_FILE_PATH}/E_" + NFStringUtility::Capitalize(file) + "_s.cpp \" --dst=${NEW_PROTOCGEN_FILE_PATH}/\n";
