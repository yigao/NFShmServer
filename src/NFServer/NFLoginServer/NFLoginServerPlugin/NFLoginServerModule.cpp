...
/*    bool flag = false;
    if (flag == false)
    {
        ...
        FindModule<NFICoroutineModule>()->MakeCoroutine([this](){
            ...
            for(int i = 0; i < TEST_SERVER_SEND_MSG_FRAME_COUNT; i++)
            {
                int iRet = FindModule<NFIMessageModule>()->GetRpcService(NF_ST_LOGIN_SERVER, NF_ST_WORLD_SERVER, 0, proto_ff::NF_RPC_SERVICE_GET_SERVER_INFO_REQ, request, respone);
                if (iRet != 0)
                {
        ...
        for(int i = 0; i < TEST_SERVER_SEND_MSG_FRAME_COUNT; i++)
        {
            ...
            proto_ff::RpcRequestGetServerInfo request;
            request.set_server_id(pConfig->ServerId);
            FindModule<NFIMessageModule>()->GetRpcService(NF_ST_LOGIN_SERVER, NF_ST_WORLD_SERVER, 0, proto_ff::NF_RPC_SERVICE_GET_SERVER_INFO_REQ, request, [](int rpcRetCode, proto_ff::ServerInfoReport& respone){
                if (rpcRetCode != 0)
                {
                    NFLogError(NF_LOG_SYSTEMLOG, 0, "GetRpcService proto_ff::NF_RPC_SERVICE_GET_SERVER_INFO_REQ Failed!");
                    return;
