...
bool NFCLoginServerModule::Awake()
{
	...
	if (pConfig)
	{
		...
		{
			NFLogInfo(NF_LOG_SYSTEMLOG, 0, "login server listen failed, serverId:{}, ip:{}, port:{}", pConfig->ServerId, pConfig->ServerIp, pConfig->ServerPort);
			return false;
		}
...
int NFCLoginServerModule::OnLoginSocketEvent(eMsgType nEvent, uint64_t unLinkId)
{
	...
	if (nEvent == eMsgType_CONNECTED)
	{
		...
		//NFLogInfo(NF_LOG_SYSTEMLOG, 0, "server:{} connect login server success", NFServerIDUtil::GetBusNameFromBusID(fromBusId));
	}
...
int NFCLoginServerModule::OnHandleServerDisconnect(uint64_t unLinkId)
{
	...
	if (pServerData)
	{
		...
		NFLogError(NF_LOG_SYSTEMLOG, 0, "the server disconnect from login server, serverName:{}, busid:{}, serverIp:{}, serverPort:{}"
...
int NFCLoginServerModule::ConnectMasterServer(const proto_ff::ServerInfoReport& xData)
{
    ...
    {
        NFLogError(NF_LOG_SYSTEMLOG, 0, "I Can't get the Login Server config!");
        return -1;
    }
...
int NFCLoginServerModule::OnMasterSocketEvent(eMsgType nEvent, uint64_t unLinkId)
{
	...
	if (nEvent == eMsgType_CONNECTED)
	{
		NFLogDebug(NF_LOG_SYSTEMLOG, 0, "login server connect master success!");
	...
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "login server disconnect master success");
	}
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "-- end --");
	return 0;
}
...
int NFCLoginServerModule::OnHandleMasterOtherMessage(uint64_t unLinkId, NFDataPackage& packet)
{
	...
	NFLogWarning(NF_LOG_SYSTEMLOG, 0, "master server other message not handled:,msg:{},ip:{}", packet.ToString(), ip);
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "-- end --");
	return 0;
}
...
int NFCLoginServerModule::OnHandleProxyRegister(const proto_ff::ServerInfoReport& xData, uint64_t unlinkId)
{
	...
	NFLogInfo(NF_LOG_SYSTEMLOG, 0, "Proxy Server Register Login Server Success, serverName:{}, ServerId:{}, ip:{}, port:{}", pServerData->mServerInfo.server_name(), pServerData->mServerInfo.server_id(), pServerData->mServerInfo.external_server_ip(), pServerData->mServerInfo.external_server_port());
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "-- end --");
	return 0;
}
...
int NFCLoginServerModule::OnProxyAgentServerSocketEvent(eMsgType nEvent, uint64_t unLinkId)
{
	...
	if (nEvent == eMsgType_CONNECTED)
	{
		NFLogDebug(NF_LOG_SYSTEMLOG, 0, "login server connect proxy agent server success!");
		RegisterProxyAgentServer(unLinkId);
	}
	else if (nEvent == eMsgType_DISCONNECTED)
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "login server disconnect proxy agent server");
	}
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "-- end --");
	return 0;
}
...
int NFCLoginServerModule::OnRouteAgentServerSocketEvent(eMsgType nEvent, uint64_t unLinkId)
{
	...
	if (nEvent == eMsgType_CONNECTED)
	{
		NFLogDebug(NF_LOG_SYSTEMLOG, 0, "login server connect route agent server success!");

		RegisterRouteAgentServer(unLinkId);
	}
	else if (nEvent == eMsgType_DISCONNECTED)
	{
		NFLogError(NF_LOG_SYSTEMLOG, 0, "login server disconnect route agent server success");
	}
	NFLogTrace(NF_LOG_SYSTEMLOG, 0, "-- end --");
	return 0;
}
